...
\begin{defn} 
We say that the statement or condition $P(x)$ for $x$ in an interval $I$ holds \emph{almost everywhere} (a.e.) if 
\[
    \{ x \in I \; | \; P(x) \text{ is false} \} 
    \;\; \text{is measure zero}
\]
\end{defn}
...
\begin{thm}
Any closed interval $[a,b]$ is of measure zero.
\end{thm}
\begin{proof}
...
\begin{defn} 
For a bounded function $f:[a,b]\rightarrow\mathbb{R}$ and nonempty $I\subset[a,b]$, we define the the \emph{oscillation of $f$ on $I$}, denoted $\Omega_f(I)$, as
\[
    \Omega_f(I) := \sup_I f - \inf_I f 
    = \sup_{x,y\in I} |f(x)-f(y)|
\]
We define the oscillation at a particular point $x\in I$ as 
\[
\Omega_f(x) = \inf_{I\ni \; x} \Omega_f(I)
\]
\end{defn}
...
\begin{prop}
Note that we can write $U(f,P)-L(f,P) = \sum^n_{i=1} \Omega_f([x_{i-1}, x_i]) \Delta x_i$.
\end{prop}
...
\end{align*}
(*) Now the next step is a big deal, because the set $X$ could potentially be uncountable, but we're going to write it as a countable union of sets: 
\begin{align*}
    X &= \bigcup_{\delta>0} \{x \; | \; \Omega_f(x)\geq \delta \} \\
    &= \bigcup^\infty_{n=1} \{x \; | \; \Omega_f(x)\geq 1/n \} =: \bigcup^\infty_{n=1}  X_n
\end{align*}
This works because $X$ is a union of points whose oscillation $\Omega_f(x)$ is non-zero. And by the Archimedian property, if we take $n\rightarrow \infty$ so that $1/n$ is sufficiently small, we'll capture \emph{all} of those points where $\Omega_f(x)>0$ eventually. In fact, as $n\rightarrow\infty$, size of our sets $X_n$ grows to include more and more points. On top of that, $X_n\supset X_m$ for $n>m$. Given all of these facts, we then know that the union of those sets will equal $X$.

So now the task is to show that $X_n$ is measure zero for all $n$.
...
We've seen in the section about sequences of functions that we can't always exchange the limit and the integral sign. In particular, there are sequences $\{f_n\}\rightarrow f$ such that each $f_n \in \mathscr{R}_\alpha([a,b])$, but $f \not\in \mathscr{R}_\alpha([a,b])$.

This stems from a deficiency in Riemann integrals arising from the relatively reigid structure of intervals---the sets over which we integrate. So this chapter will be about replacing integrals with more general ``measurable sets.'' 
...
If we're constructing a general theory from scratch, there's a number of desireable features that we would like:
\begin{enumerate}
...
\begin{rmk}
This wishlist is incompatible if our number system consists exclusively of rationls. This is because $m(\{p\}) = 0$, for all single rationals $p\in\mathbb{Q}$. Then $m(\mathbb{Q})=\sum m(\{p_n\}) = 0$. So the measure of all sets is zero, giving us no useful theory.
...
\begin{rmk}
    It also turns out that you can't satisfy the wishlist if you try to define a measure for the powerset,$\mathscr{P}(\mathbb{R})$. We'll need to consider a proper subset of $\mathscr{P}(\mathbb{R})$.
\end{rmk}
...
\begin{defn} 
($\sigma$-algebra)
An algebra $\mathscr{A}$ is a \emph{$\sigma$-algebra} if it's closed under countable unions,  i.e. whenever $\{A_n\}^\infty_{n=1}$ is a sequence in $\mathscr{A}$, then $\bigcup^\infty_{n=1} A_n \in \mathscr{A}$. As a consequence, in a $\sigma$-algebra, $\bigcap^\infty_{n=1} A_n \in \mathscr{A}$ as well.
\end{defn}
...
\begin{defn} 
Let $X$ be a set and $U$ be a $\sigma$-algebra on $X$. Then the couplet $(X, U)$ is called a \emph{measurable space}.
\end{defn}
...
\begin{defn} 
    A \emph{non-negative measure} is a measure that satisfies ``countable additivity'' for all sequences $\{A_n\}_{n=1}^\infty$ in $\mu$. That means $\mu\left(\bigcap^\infty_{n=1}A_n\right) = \sum^n_{n=1} \mu(A_n)$, where the $A_i$ are pairwise disjoint.
\end{defn}

\begin{defn} 
The triplet $(X,U,\mu)$ is called a \emph{measure space}. They have the following properties:
\begin{enumerate}
    \item $\mu(\emptyset)=0$. To prove this, write for any non-empty set $A=A\cup\emptyset\cup\emptyset\cdots$. Then use countable addivity.
    \item Finite additivity, which follows from countable additivity and a similar argument as 1).
\end{enumerate}
\end{defn}
