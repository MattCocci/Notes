...
\end{thm}
\begin{proof}
(Show $\mathscr{M}$ is an Algebra) First, it's stupidly obvious, that if $S\in\mathscr{M}$, then $S^c\in\mathscr{M}$ too, and second, that $\emptyset, X\in\mathscr{M}$. 
...
\begin{align}
    ...
    +\mu^*(\tilde{T}\cap S_2^c) 
\end{align}
Before simplifying this last expression, we use Caratheodory's criterion to simplify 
\begin{align*}
    \mu^*(\tilde{T} \cap S_2)
    +\mu^*(\tilde{T}\cap S_2^c) =
...
\[
    S_1 = A_n \qquad 
    S_n = A_n \setminus \left(\bigcup^{n-1}_{n=1} S_{n-1}\right)
    \quad \text{$n>1$}
\]
