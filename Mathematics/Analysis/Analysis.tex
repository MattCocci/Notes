...
\begin{enumerate}
\item The proof is very easy once recall the definition of $m^*(A)$. For an open set $U\supset A$, clearly by monotonicity $m(U)\leq m^*(A)$, which will hold taking the sup over $U$ as well. To prove the opposite direction, recall the definition of $m^*(A)$:
\[
    m^*(A) := 
...
% G_delta
\item For all $n$, use Part (1) to choose an open set $U_n\supset A$ such that 
\[
    m(U_n) \leq m^*(A) + \frac{1}{n}
\]
Now, let $G:=\bigcap^\infty_{n=1} \supset A$. Clearly, $G\in G_\delta$, and by monotonicity and the fact $G$ is an intersection, we know that 
\begin{align*}
    m^*(A) \leq m(G) &\leq m(U_n) \\
    &\leq m^*(A) + \frac{1}{n}
\end{align*}
Sending $n\rightarrow\infty$, then we see that $m(G)=m^*(A)$.
\end{enumerate}
Next, we prove the results for a measurable set $E\in\mathscr{M}$.
\begin{enumerate}
\item First, use Part (1) of Theorem \ref{lebreg} for arbitrary sets to choose an open set $U \supset E$ such that  
\begin{align*}
    m(U) = m(E) + \varepsilon
\end{align*}
...
We now reconsider the Fundamental Theorem of Calculus in the context of the Lebesgue Integral. The resulting theorem will be slightly different than what we saw for the Riemann Integral. Some of the results in this chapter will be swept to Appendix \ref{app.lebftc} so as not to obscure the main results for the FTC.

\begin{defn} 
For a Lebesgue integrable function $f$, the \emph{average function} of $f$ over $h$ at $x$ is defined
\[
    (A_h f)(x) := \frac{F(x+h)-F(x-h)}{2h} = \frac{1}{2h} \int_{[x-h,x+h]} f\; dm
    \qquad h>0
\]
\end{defn}
...
\begin{thm} Again, the theorem has two parts.
\begin{itemize} 
\item[\emph{I.}] If $f$ is absolutely continuous, then $f$ is differentiable almost everywhere, $f'\in\mathscr{L}([a,b])$, and 
\[
...
\begin{ex} (Cantor's Devil Staircase) To see why we need an \emph{absolutely} continuous function in the FTC for Lebesgue, Cantor's Devil Staircase provides a function that is monotonic, continuous, differentiable almost everywhere, and surjective with $f([0,1])=[0,1]$. See Definition \ref{devstair} in appendix \ref{CantorSet} for details. As a preview, here is the plot:
...
\begin{proof}[Proof of Theorem \hyperlink{maxmeas.thm}{\ref*{maxmeas.thm.n}}]
...
\begin{proof}[Proof of Theorem \hyperlink{maxineq.thm}{\ref*{maxineq.thm.n}}]
\end{proof}
