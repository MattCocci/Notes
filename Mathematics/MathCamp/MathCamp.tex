...
(together with the Contraction Mapping Theorem and some assumptions)
will let us assert existence, uniqueness, and properties (increasing,
concave, etc.) of a solution $v_B$ to functional Equation~\ref{eq:vB}.
...
\subsubsection{Principal of Optimality: Equivalence of Sequential and
Recursive Reps}
...
\begin{align}
  ...
  \beta v_B(x')
\end{align}
This section merely hopes to establish when we can refer to $v^*$ and
$v_B$ interchangeably given properties of the problem primitives
$(X,F,\beta,\Gamma)$, where
\begin{itemize}
  ...
  \item $F:A\ra \R$: the period return function where $A$ is the
    ...
    \end{align*}
\end{itemize}
For convenience, we also define objects
\begin{itemize}
...
We lay out and label the following assumptions:
\begin{enumerate}
  ...
  \item \emph{(Transversality Condition)}:
    ...
    matter'' no matter the starting $x_0$ and subsequent path
    $\{x_{t+1}\}_{t=0}^\infty$.
...
\clearpage
\subsubsection{Solving by Guess and Verify}
