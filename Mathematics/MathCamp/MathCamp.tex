...
Note that  the Bellman value function $v_B$ is a fixed point of the
contraction map $T$ defined above, i.e. $T[v_B] = v_B$.
\end{defn}
\begin{rmk}
We don't strictly need bounded functions $f:\R\ra\R$. We might take a
closed subset $X\subseteq\R$ as the domain and consider $f:X\ra\R$,
which will bounded if $f$ continuous.
\end{rmk}
...
\begin{enumerate}
  ...
  \item \emph{(Transversality Condition)}:
    Given function $v_B$,
    %for all $x_0\in X$ and all feasible sequences
    \begin{align*}
  ...
  \item
    ...
    The function $F:A\ra \R$ is continuous and bounded and
    $\beta\in (0,1)$.
...
\begin{enumerate}
  ...
  \item
    ...
    g(\hat{x}_t)$ for $\hat{x}_0=x_0$ given is an optimal,
  ...
  \item
    ...
    \begin{enumerate}
      \item $T$ maps the space of bounded continuous
        functions $\sB(X,\R)$ into itself, i.e.\
        we have
        $T\left[\sB(X,\R)\right]\subseteq\sB(X,\R)$.
      \item
        $T$ has unique fixed point $v$, and
        $\forall v_0 \in \sB(X,\R)$, we have
