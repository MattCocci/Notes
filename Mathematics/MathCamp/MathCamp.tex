...
\subsection{Solving System of Differential Equations}

\begin{prop}
\label{prop:sysdiffeq}
Suppose we have the following linear differential equation system
\begin{align*}
  \dx(t) = Ax(t)
  \qquad x(t)\in\Rn
\end{align*}
If $A$ permits an eigenvalue decomposition $A=V\Lambda V^{-1}$, then the
system solution is given by
\begin{align*}
  x(t) = \sumin c_i e^{\lambda_i t}v_i
  \quad\text{where}\quad
  \hat{x}=\sumin c_iv_i
\end{align*}
where the $\lambda_i$ are eigenvalues of $A$, $v_i$ the corresponding
eigenvectors, and $x(0)=\hat{x}$ the initial condition needed to pin
down the $n$ unknown constants $c_i$.
\end{prop}

\begin{cor}
Suppose that the first $k\leq n$ eigenvalues of $A$ have negative real
parts, $\Re(\lambda_i)<0$ for $i=1,\ldots,k$.\footnote{%
  That they are the \emph{first} $k$ is wlog.
}
Then $\exists$ a $k$-dimensional subspace of $\Rn$ denoted by $X$ such
that
\begin{align*}
  x(0)\in X
  \quad\implies\quad
  x(t) \ra 0\in\Rn
\end{align*}
In words, if the initial condition specifies that solution $x(t)$ starts
at some $x(0)\in X$, the solution will eventually converges to the zero
vector. This space $X$ is defined as
\begin{align*}
  X =
  \left\{
    x\in\Rn \;\big|\;
  ...
  \right\}
\end{align*}
In words, $x\in X$ if and only if it can be written as the sum of the
first $k$ eigenvectors, which have eigenvalues satisfying
$\Re(\lambda_i)<0$ by assumption.
So we are ruling out $x$ having any component pointing in the direction
of eigenvector $v_j$ if $\Re(\lambda_j)\geq 0$.
\end{cor}

\begin{proof}
%(Proposition~\ref{prop:sysdiffeq})
Using eigen-decomposition $A=V\Lambda V^{-1}$, we can rewrite the
system
\begin{align*}
...
\begin{align}
  ...
  %\quad\forall i>k
\end{align}
And clearly $z(t)=V^{-1}x(t)\ra 0$ if and only $x(t)\ra 0$.
So Statement~\ref{diffeqconvcond} holds if we replace $z(t)$ with
$x(t)$.
And if a solution $x(t)$ satisfies this condition, it must satisfy this
condition at all $t$, including $t=0$.
Hence, the construction of space $X$.
\end{proof}
...
\emph{function of time} $y_t:\R_+\ra Y$ that maximizes
\begin{align}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Set $\dot{x}_t=\dot{\lambda}_t=0$ in those expressions and solve
    for steady state $x^*$ and $y^*$
  \item
    If $\lambda_t$ and $x_t$ scalar, draw phase diagram in
