...
\begin{enumerate}
  \item \emph{Feasible Point}: Any point $x\in\mathcal{D}$ that
    satisfies all of the inequality and equality constraints.
  \item \emph{Feasible Set}, $\mathscr{F}$: The set of all feasible
    points.
  \item \emph{Optimal Value}, $p^*$: The maximal value of the problem,
    $\sup_{x\in \mathscr{F}} f_0(x)$
  \item \emph{Optimal Point}, $x^*$: Any feasible point
    $x^*\in\mathscr{F}$ such that $f_0(x^*)=p^*$.
\end{enumerate}

I will collectively refer to this entire problem as
Problem~\ref{prob:max} or the ``Canonical Optimization Problem'', and
solving it will be the goal of this entire section. I will proceed by
starting with some general results about extrema, which correspond to
Problem~\ref{prob:max} without any constraints---simple unconstrained
optimization over a function's entire domain. I will then add on the
equality constraints, and then the inequality constraints.
...
differentiability, concavity, convexity, etc. Of course, they will
occasionally factor in later, since we often use continuity to ensure
existence of a maximum or concavity/convexity to ensure uniqueness.
\emph{However}, we will also derive many useful results where we impose
no assumptions on the problem. So any assumptions will generally be made
very explicit, and in each iteration of
Problem~\ref{prob:max}---unconstrained, equality constrained, equality
and inequality constrained---I will present a mix assumption-free and
assumption-heavy results.
...
Note that I do not assume that functions are differentiable. Unless I
specifically say the function is differentiable, it holds generally.
...
Suppose that $f:\Rn\rightarrow\R$ is a differentiable function. The
point $x^*\in\Rn$ is called a \emph{critical point} of $f$ if
\begin{align*}
  \nabla f(x^*) = 0
  \iff
  f_i(x^*)=0
...
\end{align*}
For $f$ is \emph{strictly} concave, $x^*$ is is a \emph{unique} global
max.
\end{prop}
\begin{proof}
...
multipliers}. Overall, $\Lg$ is a function of $n+m+k$ variables since
...
\begin{align*}
  ...
  \end{cases}
\end{align*}
In other words, if you violate the constraints, you get an infinite
penalty.  So in trying to maximize objection function of our new
Problem~\ref{prob:linapprox}, we see that the optimization will result
in an $x\in\mathcal{D}$ that satisfies the constraints and avoids the
infinite penalties.

The Lagrangian is quite similar. But instead of using an infinite
penalty, it introduces a \emph{linear} penalty for some vectors
$\lambda$ and $\mu$.

\begin{defn}{(Lagrange Dual Function)}
Given Lagrangian $\Lg(x,\lambda,\mu)$, define the \emph{Lagrange dual
function}, or simply the \emph{dual}, as
\begin{align*}
  g(\lambda,\mu) = \sup_{x\in\mathcal{D}} \; \Lg(x,\lambda,\mu)
  = \sup_{x\in\mathcal{D}}
    \left\{ f_0(x)
    + \sum^m_{i=1} \lambda_i f_i(x)
    + \sum^k_{i=1} \mu_i g_i(x)
...
The Lagrange dual function is hugely important. While we generally have
no guarantees about the shape of the objective function or the
Lagrangian (unless we impose restrictive assumptions on the functions
$f_0$, $f_i$, and $g_i$), we will see that the Lagrange dual function is
\emph{always} convex for canonical Problem~\ref{prob:max}. So we can
apply many of the nice results that we saw in
Section~\ref{sec:extrema-concave} for extrema of convex and concave
functions without assuming anything.  Moreover, we can use the dual to
bound from above the value of the objective function at the optimum.
This follows because the dual is a family of upper bounds for the
objective function, parameterized by the vectors $\lambda$ and $\mu$.
Now let's make all of these formal statements.

\begin{thm}{\emph{(Dual as Upper Bound)}}
\label{thm:dual-bound}
For any feasible points $x\in\mathscr{F}$ in the domain $\mathcal{D}$,
and $\lambda\geq 0$, we have
\begin{align}
...
\begin{proof}
Take feasible point $x\in \mathscr{F}$. By the definition of feasibility
it satisfies the constraints. So first,
\begin{align*}
  g_i(x)=0 \; \forall i
  \implies
  \sum_{i=1}^k g_i(x) = 0
...
\end{align*}
So in adding this two things to $f_0(x)$, we can only increase the value
of the sum:
\begin{align*}
  f_0(x) \leq f_0(x)
...
function $g(\lambda,\mu)$ is \emph{convex}, even without any additional
assumtions on functions $f_0$, $f_i$, or $g_i$.
\end{thm}

\subsection{The Dual Canonical Optimization Problem}

Suppose you have Canonical Optimization Problem~\ref{prob:max}. You can
always write down and solve the \emph{dual} problem which, not
surprisingly, involves working with the dual function for the problem.
Often, this is much easier to solve, so I now lay out that problem
explicitly, just like Problem~\ref{prob:max}.
...
\paragraph{Terminology and Notation}
\begin{enumerate}
  \item \emph{Dual Feasible}: Pair $(\lambda,\mu)$ is ``dual feasible''
    if $\lambda\geq 0$.
  \item \emph{Dual Value}, $d^*$: The optimum of
    Problem~\ref{prob:dual}, $d^* = \inf_{(\lambda,\mu), \lambda\geq 0}
    g(\lambda,\mu)$
  \item \emph{Weak Duality}, $p^*\leq d^*$: This follows from
    Theorem~\ref{thm:dual-bound}, where $p^*$ is the optimal point of
    corresponding canonical Problem~\ref{prob:max}.
  \item \emph{Strong Duality}, $p^*=d^*$: This does not hold in general,
    but we care about precisely those cases where it does.
\end{enumerate}
Now let's see some theorems about when strong duality holds.
...
\subsection{The Concave Programming Problem}

In this section, I cover a special case of Problem~\ref{prob:max} that
is particularly well behaved. Specifically, the problem is as before
with additional assumptions on the constraints:
\begin{alignat}{3}
...
% - KKT Conditions: These are necessary conditions for an optimum
% - When some x* satisfies the KKT conditions
%   - If x* is regular and a soln
% - Strong Duality: Slater and regularity
% - Regularity
% - How to solving this shit

% Dynamic programming
% - correspondences
% - hemi continuity
% - canonical problem: stokey vs sarget notation
% - Bellman Operator
