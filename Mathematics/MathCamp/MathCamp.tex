...
\begin{enumerate}[label=(\roman*)]
  \item \emph{Closure}:
    %For all $g_{a_1},g_{a_2}\in\calG$,
    %the function $g_{a_1}\circ g_{a_2}\in\calG$.
  ...
  \item \emph{Identity Element}:
    ...
    %Given that $\calG$ is a collection of transformations,
    %this is the only way to satisfy the more general requirement
    %that $g_i\circ g_a = g_a \circ g_i=g_a$ for all
    %$g_a\in\calG$ in our original general definition of a group

  \item \emph{Inverse Element}:
    For each $g_a\in\calG$, there exists another element
    $g_{a}^{-1}\in\calG$ that undoes the transformation,
    %$g_a \circ g_a^{-1}=g_{a}^{-1}\circ g_{a} =
    %g_i$ or, equivalently,
    $g^{-1}_a(g_{a}(y))=y$.
\end{enumerate}
...
\chapter{Functional Analysis \& Linear Algebra for Vector Spaces}

This section considers vector spaces and develops important results in
functional analysis, which is a generalization of linear algebra.
Therefore, this section will be organized as a functional analysis
exposition, with linear algebra as corollary and special case.

To give an overview, linear algebra is the study of \emph{linear maps}
between finite-dimensional vector spaces, e.g. $\Rn$ and $\C^m$.
...
study of, decomposition of, and operations on matrices, which is the
traditional focus of linear algebra.
Functional analysis the study of linear maps between \emph{general}
vector spaces.
We don't study matrices that encode linear maps because, when dealing
with infinite-dimensional vector spaces, a matrix is not enough to fully
encode a linear map.
And so, in functional analysis, we work with the linear maps
directly.

Recall linear maps are the correct notion of a morphism
(structure-preserving mapping) between vector spaces because they
preserve the very linearity of the underlying vector spaces.


In functional analysis, we study the following function spaces
(decreasing in generality):
\begin{itemize}
  \item Topological vector spaces
  \item Normed vector spaces
  \item Banach spaces (complete normed vector spaces)
  \item Hilbert spaces (complete inner product spaces)
\end{itemize}
Because Euclidean space, equipped with the usual Euclidean norm, is a
finite-dimensional Hilbert space, all functional analysis results about
linear maps between Hilbert spaces will carry over into linear algebra.
...
\begin{defn}(Span of a Set of Vectors)
...
\clearpage
\subsection{Linear Maps: Morphisms Between General Vector Spaces}
...
If there exists a bijective linear map $f$ between two vector spaces $V$
and $W$, then map $f$ is an isomorphism and vector spaces $V$ and $W$
are isomorphic.
\end{thm}
...
\clearpage
\subsection{%
  Matrices: Representors of Linear Maps between Finite-Dim.\ Vector Spaces
}
...
\begin{prop}
\emph{(Matrix Representation of any Linear Map between Finite-Dim Vector Spaces)}
Given bases for finite-dimensional vector spaces $V$ and $W$ over the
same field $F$,
any linear map $f : V \ra W$ can be uniquely represented by a matrix.
Conversely, any $m \times n$ matrix gives rise to a linear map between
$F^n$ and $F^m$.
\end{prop}

\subsubsection{Basic Terminology and Definitions}
...
\subsubsection{Fundamental Subspaces \& Rank-Nullity Theorem}
...
\begin{align*}
  ...
  \operatorname{span}(A)
\end{align*}
Note: some texts use ``range'' to refer to the image, but
now considered poor terminology.
\end{defn}
...
Suppose $f:V\ra W$ is a linear map between finite-dim.\
vector spaces.
Kernel of $f$ a subspace of $V$; image of $f$ a subspace of $W$.
\end{thm}
...
\end{align*}
We can and should also think of rank as the the number of linearly
independent columns or, alternatively, rows of $A$.\footnote{%
  Whether you check rows or columns doesn't matter, those numbers will
be equal.
}
\end{defn}
...
\begin{align*}
  ...
  \operatorname{dim}(V)
\end{align*}
Hence the rank $r$ is such that $r \leq n$ and $r\leq m$.
\end{thm}
...
\begin{defn}{(Invertible and Matrix Inverse)}
Matrix $A$ is \emph{invertible} if there exists a matrix $A^{-1}$ such
that $A A^{-1} = A^{-1}A=I$
\end{defn}

\begin{prop}(Invertible $\implies$ Isomorphism)
If matrix $A$ is an invertible matrix from some finite-dimensional
vector space into itself, then the linear map associated with the matrix
is an isomorphism.
\end{prop}
\begin{proof}
Matrices already encode linear maps, the proper notion of a morphism
between vector spaces.
If the matrix is invertible, then it is an invertible morphism, hence an
isomorphism.
\end{proof}
...
\clearpage
\subsection{Dual of Vector Space: Collection of Linear Maps from $V$ into $F$}
...
%Hence, linear combinations of eigenvectors are also eigenvectors.
\end{defn}
%\begin{proof}
...
The \emph{geometric multiplicity} of eigenvalue $\lambda$, denoted
$g(\lambda)$ is the dimension of the eigenspace associated with
$\lambda$:
\begin{align*}
  g(\lambda):=\dim(V_\lambda)
\end{align*}
Equivalently, it is the number of linearly independent eigenvectors in
$V_\lambda$.
\end{defn}
...
The relationship between algebraic and geometric multiplicity is
...
\begin{thm}\emph{(Eigenvalue Decomposition, Diagonalizable Matrices)}
\label{thm:eigendecomp}
Given matrix $A\in\Rmm$, let $\{\lambda_i\}$ denote its eigenvalues.
If and only if the geometric and algebraic multiplicity of each
eigenvalue coincide,
\begin{align}
  \label{cond:dimmult}
  g(\lambda)
  ...
  m(\lambda)
\end{align}
so that there exist $n$ linearly independent eigenvectors of $A$,
we can diagonalize the matrix $A$ according to the following
\emph{eigendecomposition} or \emph{eigenvalue decomposition}:
\begin{align*}
  A = V \Lambda V^{-1}
\end{align*}
where $\Lambda$ is a diagonal matrix of the eigenvalues of $A$, and $V$
is a matrix whose columns are the corresponding eigenvectors of $A$.
\end{thm}
\begin{note}
...
$V$.
Since we assumed the eigenvectors are linearly independent,
$V$ full rank and invertible, so $V^{-1}$ well-defined.
...
\begin{align*}
  ...
  = V\Lambda
\end{align*}
Now for the final statement about mulipliticities.
If the statement is true,
there will be $m$ linearly independent eigenvectors for
...
\clearpage
\subsection{%
  Continuous/Topological Dual of Normed Vector Space \& the Dual Norm
}
...
\clearpage
\subsection{Continuous/Topological Dual of an Inner Product (Vector)
...
\clearpage
\subsection{Adjoint of an Operator, Hermitian \& Symmetric Matrices, Definiteness}
...
is invertible and its inverse is given by its adjoint, i.e.
$A^{-1}=A^*$.
\end{defn}
...
\begin{defn}(Transpose and Hermitian Transpose of a Matrix)
...
denoted by $A^*$ and is the conjugate transpose of $A$ (i.e. take rows
of $A$ and conjugate to get columns of $A^*$).
\end{defn}
...
\begin{align*}
  ...
  \quad
  \sum_{i=1}^m
  (A_{i,\cdot}\,x)\overline{y}_i
  &=
  \sum_{j=1}^n
  x_j \overline{(A^*_{j,\cdot}\,y)}
  \\
  \iff
  \quad
  \sum_{i=1}^m
  ...
  \left(
    \sum_{i=1}^m
    \overline{A^*_{ji}}\,\overline{y}_i
  \right)
  \\
  \sum_{i=1}^m
  \sum_{j=1}^n A_{ij}x_j\overline{y}_i
  &=
  \sum_{i=1}^m
  \sum_{j=1}^n
  \overline{A^*_{ji}}x_j\,\overline{y}_i
\end{align*}
In order for this equality to hold (and it must by the definition of
$A^*$ as the adjoint), we need $A_{ij}=\overline{A}_{ji}^*$, implying
that $A^*$ is the conjugate transpose of $A$.
If $A$ is a real matrix, then it's simply the transpose.
\end{proof}


\begin{defn}{(Hermitian/Self-Adjoint and Symmetric Matrices)}
In the same way that a bounded linear operator/map is
Hermitian/self-adjoint if it equals its own adjoint, a matrix $A$ is
\emph{Hermitian/self-adjoint} if it equals its own adjoint or,
equivalently (by previous proposition), its Hermitian transpose, i.e.
$A=A^*$.

If $A$ is a real Hermitian matrix, we often instead use the term
\emph{symmetric} to denote such matrices, in which case $A=A^T$.
\end{defn}


\begin{defn}{(Unitary/Orthogonal Matrix)}
Matrix $A$ is \emph{unitary} if its inverse equals its adjoint
(Hermitian transpose), i.e. $A^{-1} = A^*$.
Matrix $A$ is \emph{orthogonal} if it is a \emph{real} unitary matrix:
$A^{-1} = A^T$ so that $AA^T = A^T A = I$.
\end{defn}

\begin{defn}{(Normal)}
Matrix $A$ is \emph{normal} if $A^* A = AA^*$.
\end{defn}

\begin{prop}
Matrices that are both Hermitian and unitary are normal.
\end{prop}
\begin{proof}
Hermitian implies $A=A^*$.
Unitary means $A^{-1}=A^*$.
Now check
\begin{align*}
  A^*A=A^{-1}A=I=AA^{-1}=AA^*
...
\clearpage
We now establish a direct correspondence between symmetric positive
semi-definite matrices and collections of vectors in inner product
spaces.
...
\subsubsection{Geometric Perspective on Projections onto a Set}

\begin{thm}\emph{(Projection Theorem: Projection onto a Set)}
\label{thm:projthm}
...
\begin{defn}(Orthogonal Projection onto a Set)
...
\clearpage
\subsubsection{Functional Analysis Perspective on Projections onto a Set}
...
\begin{enumerate}[label=\emph{(\roman*)}]
  ...
  \item Satisfies $P_C^2=P_C$
  \item Normal, $P_CP_C^*=P_C^*P_C$, by \emph{(ii)} and \emph{(iv)}
\end{enumerate}
...
characterizes/distinguishes Hilbert spaces within the broader set of
Banach spaces.
...
\subsubsection{Orthogonal \& Orthonormal Families}
...
\begin{align*}
  ...
  \lVert u_i\rVert^2
\end{align*}
%where the last equality followed since $\{u_i\}$ is an
%family.
\end{proof}
...
\begin{defn}(Orthogonal Complement, $A^\perp$)
...
\begin{prop}
\emph{(Results about $A^\perp$)}
Given an inner product space $(V,\langle\,\cdot,\,\cdot\rangle)$,
some set $A\subseteq V$ and orthogonal complement $A^\perp$, we have
\begin{enumerate}[label=\emph{(\roman*)}]
  \item The orthogonal complement $A^\perp$ is a subspace.
    If the inner product space $(V,\langle\,\cdot,\,\cdot\rangle)$ is
    complete (i.e. Hilbert space), then $A^\perp$ is also a Hilbert
    Space.
  \item $A\cap A^\perp=\{0\}$

  % Below might only hold if A is a subspace
  %\item $(A^\perp)^\perp=A$
  %\item $A\subseteq B$ implies $B^\perp \subseteq A^\perp$
  %\item $A+A^\perp = \R^n$

  \item If $A$ is a closed subspace of $V$, every $x\in V$ can be
    written uniquely as
    \begin{align*}
      x = v + w
    \end{align*}
...
\begin{itemize}
  ...
  \item[(iii)]
    This is a direct corollary of the Projection
    Theorem~\ref{thm:projthm}.
    As for uniqueness, suppose we have two $x=v_j + w_j$.
    Well then $0=v_1-v_2+w_1-w_2$ implying $-(v_1-v_2)=(w_1-w_2)$.
    They both have to equal zero because $v_1-v_2$ is in $A$ and $w_1-w_2$
    in $V^\perp$. The only point of overlap in $V$ and $V^\perp$ is
    zero.
\end{itemize}
\end{proof}

We now make statements about matrices representing linear maps between
finite-dimensional vector spaces and the fundamental subspaces of those
matrices.
...
\begin{prop}
\emph{(Orthogonal Complement of Null Space is Range of Transpose)}
...
\clearpage
\subsubsection{Projection onto Orthonormal Families: Inversion,
Parseval/Plancherel, Fourier Formulas}

Previously, we defined projections onto a nonempty closed convex set in
the Projection Theorem~\ref{thm:projthm}; now we describe projections
onto orthonormal families, or rather, the subspace generated by them.
Rather than projecting onto some arbitrary set (like in the Projection
Theorem), the additional structure of projecting onto orthonormal
families furnishes additional results.
In particular, we use Proposition~\ref{prop:projline} which considers
projection onto a line in order to write/decompose any vector as a sum
of its projection onto individual vectors in an orthonormal family or
basis.

This is useful because, intuitively, we have a canonical representation
of any vector as just a weighted average of orthonormal vectors in some
family (that we get to choose!), modulo the weights/coefficients that we
can determine according to the results in this section.

\begin{thm}
\label{thm:almostinversion}
\emph{(Finite ``Almost'' Inversion Formula, Bessel's Inequality, Parseval/Plancherel's Formula)}
...
\begin{enumerate}[label=\emph{(\roman*)}]
  \item
    \emph{(Same Inner Product)}:
    $\langle x,v_i\rangle =\langle \hat{x},v_i\rangle$
    for all $i=1,\ldots,n$
  \item
    \emph{(Error Orthogonal to Family Elements)}:
    $\langle x-\hat{x},v_i\rangle=0$ for all $i$
  \item
    \emph{(Error Orthogonal to Projection Itself)}:
  ...
  \item
    ...
    \begin{align*}
      ...
      \sum_{i=1}^n \bigl\lvert\langle x,v_i\rangle\bigr\rvert^2
    \end{align*}
    If Bessel's inequality holds with equality, the resulting expression
    is called Parseval's or Plancherel's formula.
\end{enumerate}
\end{thm}
\begin{rmk}
...
%\begin{align*}
  %\lVert x\rVert^2 \geq
  %\sum_{i=1}^{|I|}
  %\big|\langle x,v_{\alpha_i}\rangle\big|^2
%\end{align*}
...
\begin{thm}
\label{thm:inversion}
\emph{(Infinite Inversion and Parseval/Plancherel Formulas)}
...
Further, because $x$ assumed to lie in the span of the orthonormal
system, the finite Parseval/Plancherel Formula holds, i.e. Bessels'
...
For Parseval/Plancherel formula, see proof of Proposition~\ref{prop:ortholi}.
\end{proof}
...
Then
\begin{itemize}
  \item (\emph{Fourier Expansion}):
    Every $x\in V$ lies in the span of the orthonormal basis,
    and so the inversion formula~\ref{inversionformula} holds
    \begin{align}
      x
      = \sum_{\alpha\in J} \langle x,v_{\alpha}\rangle v_{\alpha}
      = \sum_{\alpha\in J} P_{A_{v_\alpha}}(x)
      \label{inversionformula2}
    \end{align}
    which, in ths case, is also called the \emph{Fourier expansion} of
    $x$, while the coefficients on the $v_\alpha$ are called
    \emph{Fourier coefficients}.

    The most canonical Fourier expansion takes $V$ as the space of all
    functions $f:\R\ra\C$ equipped with the complex $L^2$ norm.
    Then for any function $f\in V$, we can use the orthormal basis of
    functions
    $\{\phi_\alpha\}_{\alpha\in \R}=\{e^{2\pi i t \alpha }\}_{\alpha\in \R}$
    to write
    \begin{align*}
      f
      &=
      \sum_{\alpha\in \R}
      \langle f, \phi_\alpha \rangle_{L^2}
      \;
      \phi_\alpha
      \\
      \implies
  ...
  \item By Bessel's inequality, expression~\ref{inversionformula2} is
    well-defined and consists of only countably many nonzero terms.

  \item Further, Parseval/Plancherel's identity holds, which shows how we can
    compute the norm of $x$ using the inner products of $x$ with the
    elements of the orthonormal basis.

    In the canonical Fourier expansion, this means
    \begin{align*}
      \lVert f\rVert_{L^2}^2
      &
      =
      \sum_{\alpha\in \R}
      |\langle f, \phi_\alpha \rangle_{L^2}|^2
      \\
      \iff
      \qquad
      \int_{-\infty}^\infty
...
\begin{prop}
\emph{(Finite Gram-Schmidt Process: How to Orthogonalize and Span)}
...
\end{prop}
\begin{cor}
\emph{(Finite Gram-Schmidt Theorem: Orthogonalizing a Given Basis)}
...
%\begin{cor}
%An $n$-dimensional real or complex inner product space is isomorphic to
%$\R^n$ or $\C^n$ respectively.
%\end{cor}
...
\emph{(Parseval/Plancherel Holds for Orthornormal Family $\implies$ Orthonormal Basis)}
Suppose we have an orthonormal set $\{v_\alpha\}_{\alpha\in J}$ such
that the Parseval/Plancherel formula~\ref{plancherel} holds for every
$x\in V$.
...
\clearpage
\subsection{Spectrum \& Eigenvalue Decomp.\ of Self-Adjoint/Hermitian Operators}
...
Once we move to Hilbert spaces in which we have the notion of
\emph{self-adjoint}/\emph{Hermitian} linear maps, things become a lot
richer and we obtain an entire spectral theory with many uses and
results.
...
finite-dimensional real Euclidean spaces or \emph{Hermitian matrices}
for finite-dimensional complex Euclidean spaces.
The key results of spectral theory concern when a linear operator
$T:V\ra V$ or matrix $A:\Cn\ra\Cn$
can be \emph{diagonalized}, e.g. a matrix can represented as diagonal
matrix in some space.
This is useful because its much easier to work with diagonal
representations.
As a result of spectral theory, we always get a canonical decomposition
called the \emph{spectral decomposition}, \emph{eigendecomposition}, or
\emph{eigenvalue decomposition} of the underlying vector $V$ space on
which the linear operator $T:V\ra V$ acts.
This decomposition agrees with the eigenvalue decomposition earlier, but
is always guaranteed to exist for self-adjoint operators; previously, we
did not have that guarantee.

These results can be generalized to normal operators, i.e.
statisfying $T^*T=TT^*$, rather than just self-adjoint/Hermitian
operators satisfying $T=T^*$.
...
All eigenvalues of a self-adjoint/Hermitian map are real.
\end{thm}
\begin{proof}
Let $T=T^*$ denote the self-adjoint map with some eigenvector $v$ and
associated eigenvalue $\lambda$, hence $v$ satisfies $Tv=\lambda v$.
Plug this into the definition of self-adjoint/Hermitian:
\begin{align*}
...
\emph{(Existence of Orthonormal Eigenbasis)}
If $T:V\ra V$ is a Hermitian operator, then there exists an orthonormal
basis for $V$ consising of unit-length eigenvectors
$\{q_\alpha\}_{\alpha\in J}$ of $T$, with each associated eigenvalues
$\{\lambda_\alpha\}_{\alpha\in J}$ all real by the previous theorem,
possibly exhibiting repeats.
Hence, we can decompose the operator $T$ into a linear combination of
pairwise orthogonal projections
\begin{align}
  T = \sum_{\alpha \in J} \lambda_\alpha P_{V_{q_\alpha}}
  \label{spectral}
\end{align}
where $P_{V_{q_\alpha}}$ is the projection map of any vector $v\in V$
onto the subspace $V_{q_\alpha}\subset V$ spanned by eigenvector
$q_\alpha$.
This decomposition is called the \emph{spectral} or
\emph{eigenvalue decomposition} of $T$.
\end{thm}
\begin{rmk}
...
we can easily construct a basis from the eigenvectors of that operator.
\end{rmk}


\clearpage
\begin{cor}
\emph{(Spectral/Eigenvalue Decomposition for Finite Vector Space)}
Suppose we have a self-adjoint/Hermitian operator $T:\Cm\ra\Cm$
between finite-dimensional vector space $\Cm$ and associated Hermitian
matrix $A\in\Cmm$ encoding this operator.
Then by the previous theorem, there exists an orthonormal basis
$\{q_i\}_{i=1}^m$
consisting of $m$ (because the vector space is $m$-dimensional)
unit-length linearly independent eigenvectors.
with associated real eigenvalues $\{\lambda_i\}_{i=1}^m$, possibly
exhibiting repeats.
And in addition to Expression~\ref{spectral}, we can decompose and
diagonalize matrix $A$ as follows:
\begin{align*}
  A = Q\Lambda Q'
\end{align*}
The columns of $Q=(q_1,\ldots,q_m)$ are eigenvectors of $A$,
normalized to unit length, while
$\Lambda=\diag\{\lambda_1,\ldots,\lambda_m\}$ is a diagonal matrix
of (not necessarily distinct) corresponding eigenvalues, all of which
are real.
Hence the matrix representation of $A$ in a basis of eigenvectors is
\emph{diagonal}.
\end{cor}
\begin{rmk}
If $A$ diagonal, $Av$ just stretches vector $v$ by amount $A_{ii}$ in
the direction of the $i$th Cartesian axis.
This matrix decomposition gives a similar result, but for a different
set of axes.
In particular, if $A$ is Hermitian, this result says that the columns of
$Q$ (which are the unit-length orthogonal eigenvectors) form a
\emph{new special set of axes} (basis). And the multiplication $Av$
stretches vector $v$ by eigenvalue $\lambda_i$ in the direction of
corresponding eigenvector (and column of $Q$) $q_i$.
\end{rmk}

\begin{proof}
(\emph{Proof of Diagonal Representation})
By Theorem~\ref{thm:eigenbasis}, $\{q_i\}_{i=1}^m$ is an orthornomal
basis for $V$.
Hence we can represent any $v\in V$ as some linear combination of
elements
\begin{align*}
  v = \sum_{i=1}^m c_i q_i
\end{align*}
The constants $c_i$ are pinned down by the Inversion
Theorem~\ref{thm:inversion}
\begin{align*}
  v = \sum_{i=1}^m \langle v,q_i\rangle q_i
\end{align*}
Applying the inversion formula to $Tv\in V$, we also get
\begin{align*}
  Tv
  = \sum_{i=1}^m \langle Tv,q_i\rangle q_i
  = \sum_{i=1}^m \langle v,Tq_i\rangle q_i
  = \sum_{i=1}^m \langle v,\lambda_i q_i\rangle q_i
  = \sum_{i=1}^m \lambda_i \langle v,q_i\rangle q_i
\end{align*}
The second equality followed because $T$ is Hermitian/self-adjoint;
...
$\lambda_i \langle v,q_i\rangle q_i$ is projection of
$v$ onto unit-length eigenvector $q_i$, which is then scaled by
$\lambda_i$.
This gives exactly the desired spectral decomposition.

To justify the matrix decomposition, notice
\begin{align*}
  Av
  =
  Q\Lambda Q'v
  ...
  =
  \sum_{i=1}^n
  \lambda_i \langle q_i, v\rangle q_i
\end{align*}
%(\emph{Direct Proof of Orthogonality})
...
\emph{(Eigenvalues and Definiteness)}
Given a symmetric (real Hermitian) matrix $A\in \Rmm$,
\begin{enumerate}[label=\emph{(\roman*)}]
  \item $A$ positive semi-definite iff all eigenvalues nonnegative
  \item $A$ positive definite iff all eigenvalues strictly positive
\end{enumerate}
\end{prop}
\begin{proof}
We will make use of the eigenvalue decomposition of matrix $A$.
...
Next, examine $z^Tz=x^TAx$, now assuming additionally $x\neq 0$.
...
%Then everything is the same, we can specialize the results and notation
%to probability measures and random variables, rather than just
...
\end{enumerate}
Matrix decompositions are algorithmic and mechanical, but they often
reflect deep underlying mathematics.
For example, we can always do an eigenvalue decomposition on a symmetric
matrix, but it precisely the deep mathematical interpretation of the
spectral decomposition that makes this worthwhile.
And so we have reserved the matrix decompositions for last, so that we
can make full use of the deep theory that underlie them.



\subsection{$LU$ Decomposition}

This decomposition is used to rewrite an arbitrary square matrix (whose
inverse might take a lot of computing power to solve for) instead as the
product of upper- and lower-triangular matrices, which are exceedingly
easy to invert.

But first, a result that will be useful below: If we want to swap rows
of a matrix, we can express that as matrix multiplication. More
specifically, given $A\in\R^{m\times n}$, we can swap rows $i$ and $j$
via matrix multiplication $PA$, where $P$ is simply the identity matrix
$I_m$ with its rows $i$ and $j$ swapped. This generalizes, so if we want
to rearrange more rows of $A$, simply reorder the rows of the identity
matrix $I_m$ according to how you want the rows of $A$ ordered.
...
We care about the QR decomposition mostly for solving a system of
equations and solving the least-squares problem.
The QR decomposition makes that easier, writing $A\in\Rmn$ as the
product of an orthogonal matrix $Q$ (which has the easy inverse of
$Q^{-1}=Q^T$) and an upper triangular matrix.
With this decomposition, converting coordinate systems is easy.
...
\begin{rmk}
In addition to helping with solving (just- or overdetermined) systems of
equations,
the QR decomposition also provides useful information
...
The Schur Decomposition is an ``almost eigenvalue decomposition.''
We know that any Hermitian matrix $A$ can be written $A=Q\Lambda Q'$
where $Q$ is an unitary matrix and $\Lambda$ diagonal.
But this does not hold for all square matrices, so the Schur
decomposition replaces diagonal $\Lambda$ in the middle with an upper
triangular matrix.
...
The SVD is a majorly useful decomposition because it works for
\emph{any} matrix, including and especially rank deficient matrices.
Moreover, many properties of a matrix and the linear map it encodes can
be read off directly from its SVD.
...
\begin{enumerate}
  ...
  \item {\sl Column Interpretation}:
    ...
    of $A$ to produce $b$.
\end{enumerate}
The latter interpretation is especially powerful.
The column space of $A$---the set of all linear combinations of the
columns of $A$---is crucial to characterizing the matrix $A$.
...
To solve the system $Ax=b$, we focus in this subsection on first using
matrix decompositions on $A$ to re-express the system.
Matrix decompositions work by doing a change of coordinates to a
friendlier basis, which lets us solve $Ax=b$ in a ``nicer'' space and
coordinate system, from which we can convert back to the original basis
...
\clearpage
\subsection{Solving Full-Rank Systems}
...
\clearpage
\subsection{Solving System $Ax\approx b$ with Matrix Decompositions}
...
Now this is a very short subsubsection, but we'll see in later subsections
...
\clearpage
\section{The Canonical Optimization Problem}

\begin{defn}(Canonical Optimization Problem in Primal Form) Throughout this section, we consider optimization problems for
...
Therefore, this subsection covers how to compute numerically the argmin
...
All methods in this subsection for numerically computing $x^*$ follow the
same basic approach:
\begin{enumerate}
  ...
  \item If we didn't improve much $\lVert x_{n+1}-x_n\lVert<\varepsilon$
    ...
    until convergence.
\end{enumerate}
This subsection is all about Step 2: Choosing a sensible update rule
...
In this subsection, we derive conditions useful for implementing
...
\clearpage
\section{Particular Optimization and Machine Learning Problems}

\subsection{Rayleigh Quotient Problem}
...
\section{Discrete-Time Deterministic Dynamic Optimization}
...
\clearpage
\subsection{Canonical Sequential Formulation}
...
\clearpage
\subsection{``Classical'' Solution Method for Canonical Sequential
Formulation}
...
\clearpage
\subsection{Canonical Recursive Formulation}
...
\clearpage
\subsection{Principal of Optimality: Equivalence of Sequential \&
Recursive Reps}
...
\clearpage
\subsection{Solving the Recursive Bellman Equation for $v_B$}
...
\clearpage
\section{Continuous-Time Deterministic Dynamic Optimization}
...
\clearpage
\subsection{Hamiltonian for Solving the Canonical Sequential
Formulation}
...
\clearpage
\subsection{Canonical Hamilton-Jacobi-Bellman Recursive
Representation}
...
\clearpage
\section{Continuous-Time Stochastic Dynamic Optimization}
...
\clearpage
\subsection{Hamilton-Jacobi-Bellman Approach}
