...
\begin{enumerate}[label=(\roman*)]
  \item[(v)]
    ...
    Since $f$ bounded, let $m:=\inf_{[a,b]}f$ and $M:=\sup_{[a,b]}f$
    which are both finite.
    Hence, we have two results concerning compact interval $[m,M]$,
    which is the range of $f$ on $[a,b]$ and the relevant domain of $h$.
    \begin{itemize}
      \item As a continuous function on compact interval $[m,M]$,
        $h$ is bounded on $[m,M]$. In other words, there exists a $K$
        such that $|h(x)|\leq K$ for all $x\in[m,M]$.
    ...
    \begin{equation}
        \label{forf}
          U_\alpha(f,P)- L_\alpha(f,P) \leq \epsilon\cdot
            \delta
    \end{equation}
    Now that everything's spelled out, we're interested in showing
    Riemann integrability for $h\circ f$, which requires us to show that
    the upper and lower RS-Darboux sums are arbitrarily close. We will
    do so by breaking the sums into two parts based on how the original
    function $f$ behaves on an interval:
    \begin{align*}
    ...
    \begin{align}
        ...
        &\leq \frac{\varepsilon}{2[\alpha(b)-\alpha(a)}
            ...
            \label{final}
    \end{align} The result for the sume over $A$ comes from uniform
    integrability and Inequality~\ref{cont}. The result for the sum over
    $B$ comes from $h$ being bounded by $K$ as discussed.
    ...
    \end{align*}
    Substituting this result back into Equation \ref{final}, we have
    \begin{align*}
  ...
  \item[(vi)]
    We know that $f - g$ and $f+g \in \mathscr{R}_\alpha([a,b])$
    by (i). We also know that $h(x) = x^2$ is continuous, so then we can
    say that \[ fg = \frac{1}{4} \left[(f+g)^2 - (f-g)^2 \right]
