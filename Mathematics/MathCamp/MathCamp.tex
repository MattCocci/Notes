...
$\Lg(x,\lambda,\mu)$, define the \emph{Lagrange dual function}, or
...
$\Lg_{\lambda^*,\mu^*}(x):=\Lg(x,\lambda^*,\mu^*)$.
Equation~\ref{kkt:criticalproof} effectively says that
$x^*\in\mathcal{D}$ maximizes $\Lg_{\lambda^*,\mu^*}(x)$, so the sup is
...
\begin{align*}
  0 &= \nabla_x \Lg_{\lambda^*,\mu^*}(x^*)
  =:
  \nabla_x \Lg(x^*,\lambda^*,\mu^*)
\end{align*}
...
\begin{thm}
If canonical optimization Problem~\ref{prob:max} satisfies strong
duality ($p^*=d^*$)
\begin{enumerate}
  \item $x^*$ is a solution of Problem~\ref{prob:max} implies that there
    exist $(\lambda^*,\mu^*)$ such that $(x^*,\lambda^*,\mu^*)$ satisfy
    the KKT Conditions.
  \item $g(\lambda^*,\mu^*)=d^*$, i.e. the multipliers associated with
    $x^*$ are solve the dual problem.
  \item If Problem~\ref{prob:max} is concave, satisfying
    Problem~\ref{prob:maxconcave}, then the converse is also true. In
    other words, the KKT conditions are sufficient.
\end{enumerate}
\end{thm}
\begin{proof}
(Part 1)
...
\begin{align}
  \max_{x_t,y_t} \; &\int^{t_1}_{0} F(x_t,y_t,t) \; dt
  ...
  & x_0 \; \text{given} \notag
\end{align}
We will consider problems where the terminal time $t_1$ is finite and
...
We now look for the optimal solution by using the \emph{variational
principle} of the calculus of variations. Assume there exists some
continuous, feasible function $\hat{y}_t$.
...
\end{align*}
where $\lambda$ is called the \emph{co-state}.
...
Given $x_t,\lambda_t$, the function $y\in \argmax_{y\in Y}
H(x,y,\lambda,t)$ is a maximum satisfying the necessary conditions
\begin{enumerate}
  ...
  \item $x(0) = x_0$ given
  \item $\lambda(t_1)=0$ if $t_1< \infty$
\end{enumerate}
\end{defn}
...
% - Bellman operator and algorithm
% - Properties of the value function
% - Euler equations
% - Finish v_B -> v^*
% - Properties of the value function
% - Envelope
% - Blackwell and check bellman
