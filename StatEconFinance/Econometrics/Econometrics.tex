...
\clearpage
\subsection{Extremum Estimators, M-Estimators, Sandwich Estimators}
...
called the \emph{M-estimator}, which also nests as special cases MLE and
NLS.
...
\end{align*}
for some function $q(\bsy_n,\bstildetheta)$. For differentiable $q$,
that means the true $\bstheta$ we hope to estimate satisfies
$\E\big[q'(\bsy_n,\bstheta)\big]=\bso$ where $q'(\,\cdot\,,\bstheta)$ is
the gradient of $q$ evaluated at true $\bstheta$.
\end{defn}
...
\begin{proof}
If $\bstheta\in\R^P$, then $\bshattheta$ must satisfy the following
...
$\bsx_n$ to forecast $y_n$, i.e.\ to find some function $g(\bsx_n)$ that
does a ``good'' job predicting $y_n$. We say that $g(\bsx_n)$ is
``good'' if it minimizes the Mean-Square Error of the forecast:
\begin{align}
  \min_{g(\bsx_n)} \E\big[(y_n - h(\bsx_n))^2\big]
  =
  \min_{g(\bsx_n)} \E\left[\E\big[(y_n - h(\bsx_n))^2\big] \;|\;\bsx_n\right]
  \label{nlsmotivation}
\end{align}
Using the second expression where we iterated expectations, it can be
shown that the choice of $g(\bsx_n)$ that minimizes the MSE is the
...
Since the true population CEF necessarily minimizes the population
analog of the above objective function in
Expression~\ref{nlsmotivation}, we will fit a sample CEF or CEF
approximation to minimize the sample version in Expression~\ref{nls}.
\end{defn}
...
\begin{align*}
  ...
  \big)
\end{align*}
where $\varepsilon_n:=y_n-g(\bsx_n,\bstheta)$.
Given $\bshattheta_{NLS}$, we can estimate quantities in asymptotic
variance by
\begin{align*}
  \frac{1}{N}\sumnN
...
\end{align*}
\end{prop}
\begin{cor}(Asymptotic Distribution of the OLS Esimtator)
...
were willing to assume homoeskedasticy, the standard errors would
simplify further.
\end{cor}
\begin{proof}
Notice that the general NLS estimator is an $M$-estimator:
\begin{align*}
  \bshattheta_{NLS}
  = \argmin_{\bstildetheta\in\bsTheta}
...
\begin{align*}
  ...
  \big)
\end{align*}
We now just need to compute the derivatives of our particular $q$:
\begin{align*}
  q'(y_n,\bsx_n,\bstheta)
...
\begin{align}
  ...
  \label{nlsproof}
\end{align}
And that is enough to plug into the formulas for the desired result.
...
\end{align*}
Iterating expectations, this says
\begin{align*}
  0 &=
  -2\E\left[
  ...
  \big]
\end{align*}
In general, $g'(\bsx_n,\bstheta)\neq 0$, so this implies that
$\E[\varepsilon_n|\bsx_n]=0$. That is enough, since then
\begin{align*}
  \E\big[
  \varepsilon_n
  g''(\bsx_n,\bstheta)
...
the data. The big insight of MLE is to invert our thinking about the
data generating process and think in terms of likelihoods. In
particular, suppose we consider a random sample $\bsy_{1:N}$ which has
true joint likelihood function $f(\bsy_{1:N}|\bstheta)$. There are two
ways to think about the object $f(\bsy_{1:N}|\bstheta)$:
\begin{enumerate}[label=(\roman*)]
  \item Probability theory says that if you know $\bstheta$, you know
    the data generating process of $\bsy_{1:N}$. You can use
    $f(\bsy_{1:N}|\bstheta)$ to make probability statements about the
    distribution and expected realizations of $\bsy_{1:N}$ \emph{ex ante},
    before observing any data. In that case, $f(\bsy_{1:N}|\bstheta)$ is
    a map from some hypothetical sample realization $\bsy_{1:N}$ to the
    probablity of observing that realization if you actually draw a
    sample.

  \item The MLE approach to estimation inverts that. MLE
    says instead that \emph{you have an already-realized sample}
    $\bsy_{1:N}$, but you don't know $\bstheta$.  You therefore think
    about $f(\bsy_{1:N}|\bstildetheta)$ as a map from hypothetical
    $\bstildetheta$ (not hypothetical $\bsy_n$) to the the probability
    of \emph{having observed your realized sample $\bsy_{1:N}$}, given
    that $\bstildetheta$ was the true parameter vector. Maximum
    likelihood estimation then says to choose as $\bshattheta$ the
    vector that would have delivered the highest \emph{ex ante}
...
\subsection{MLE Definition and Properties}
...
analytically. We do so by looking at the first order conditions to the
maximization problem and solving to get the parameter(s) in terms of the
sample observations. That will generate a map from the sample to the MLE
...
that $f(\bsy_{1:N}|\bstildetheta)$ was, flat out, the wrong likelihood
...
respectively, of
\begin{align*}
  \bso = \sumnN q'(\bsy_n,\bshattheta)
  =
  \sumnN S(\bstildetheta,\bsy_n)
  \qquad
  \bso
  = \E\big[q'(\bsy_n,\bstheta)\big]
...
\begin{enumerate}[label=(\roman*)]
  \item
    ...
    \begin{align*}
      \bshattheta_{MLE}
      &=
      \argmax_{\bstildetheta} \sumnN \ln f(\bsy_{n}|\bstildetheta)
    \end{align*}
    ...
    \begin{align*}
      ...
      &\leq 0
    \end{align*}
    But this is precisely the condition we described for $\bstheta$ to
    be the maximizer.
...
\end{align}
where $\bsg:\R^P\ra \R^M$. Notice that we introduced the shorthand
notation $\bsg_n(\bstildetheta)=\bsf(\bsy_n,\bstildetheta)$.
Our goal is to estimate $\bstheta$. Quite sensibly, we can estimate
parameters by drawing a sample $\{\bsy_n\}\nN$, using them to construct
sample observations $\bsg_n(\bstildetheta)$, and then forming the sample
analog $\bsg_N$ of the moment conditions
\begin{align}
...
\end{align}
This is a function of parameter vector $\bstheta$. We will naturally
choose our estimator to be the parameter vector that sets this sample
analog equal to zero or ``close'' to zero.
...
equations:
\begin{align*}
  \bsg_N(\bshattheta) &= \bso
  \qquad\text{when}\quad
  \bsg_N,\bshattheta \in\R^P
\end{align*}
\end{defn}
...
sample analog of the moment conditions $\bsg_N(\bstildetheta)$ to zero
\emph{exactly} using some $P$-dimensional $\bstildetheta$.
Therefore, we instead look for the parameter vector that gets the
squared-weighted elements of vector $\bsg_N(\bstildetheta)$ jointly as
...
\begin{align}
  ...
  :=&\;
  N\cdot \bsg_N(\bstildetheta)' \, \bshatW \, \bsg_N(\bstildetheta)
  \notag
\end{align}
...
moment conditions:
\begin{align}
  \bshata \; \bsg_N(\bshattheta) &= \bso
  \qquad\text{where}\quad
  \bshattheta \in\R^P\quad \bshata \in \R^{P\times M}
  \label{eq:bhatmina}
\end{align}
We see that $\bshata\,\bsg_N(\bstildetheta)$ is like a new set of moment
conditions, but with the dimensionality reduced from $M>P$ to $P$ so
that we \emph{can} set this to zero for some $\bshattheta\in\R^P$.
...
out the moments in $\bsg_N(\bstildetheta)$ (or form linear combinations
...
Definition~\ref{defn:gmm} if you choose $\bshata$ based on the FOCs to
the minimization problem defined by Equation~\ref{eq:bhatmin}:
\begin{align}
  \bso
  =
  \underbrace{\frac{\partial \bsg_N'}{\partial \bstildetheta} \bshatW}_{\bshata}
  \;\bsg_N(\bstildetheta)
  \label{eq:equivrep}
\end{align}
\end{defn}
...
also \emph{efficient} GMM estimators, i.e.\ an estimator $\bshattheta$
with the lowest possible asymptotic variance.
That typically presents the following chicken and egg problem:
\begin{enumerate}[label=(\roman*)]
  \item Obtaining the efficient GMM estimator, call it $\bshattheta_2$,
  ...
  \item However, the weighting matrix $\bshatW_2$ that
    ...
    parameter vector $\bstheta$. Since we don't know the true parameter
    vector, we approximate it by a consistent estimate called
    $\bshattheta_1$.
\end{enumerate}
That's the problem: (i) requires $\bshatW_2$ to get efficient
$\bshattheta_2$, while (ii) requires some consistent estimate
$\bshattheta_1$ to get the appropriate $\bshatW_2$.
...
\begin{enumerate}
  \item Choose some arbitrary $\bshatW_1$ ($\bsI$ often works) and solve
    ...
    consistent estimator $\bshattheta_1$.
  \item Use $\bshattheta_1$ to compute the second-stage efficient
    weighting matrix $\bshatW_2$, and then use this to solve
    Expression~\ref{eq:bhatmin} again for the now efficient GMM
    estimator $\bshattheta_2$.
\end{enumerate}
