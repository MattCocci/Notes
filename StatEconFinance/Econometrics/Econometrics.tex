...
\clearpage
\subsection{Lag Operator and Lag Polynomials}
...
simply write $y_{t-1}=Ly_t$. We use powers of the lag operator to
express lags of more than one time unit:
\begin{align*}
  y_{t-k} = L^k y_t
\end{align*}
Important, $L$ is a \emph{linear} operator.
\end{defn}

\begin{defn}(Lag Polynomial)
The lag operator allows us to write linear difference equations in a
very terse way. In particular, suppose we have an ARMA($p$, $q$)
\begin{align*}
  y_t
  = \phi_1 y_{t-1}
  +\cdots
...
\end{align*}
From there, we can define \emph{lag polynomials} $\phi(\,\cdot\,)$ and
$\theta(\,\cdot\,)$ to arrive at the compact expression
\begin{align*}
  \phi(L)y_t
  &=
  \theta(L)\varepsilon_t \\
...
\end{align*}
where we think of $\phi(\,\cdot\,)$ and $\theta(\,\cdot\,)$ as
polynomials that we can evaluate at $L$ (the lag operator) or any
$z\in\C$.
\end{defn}
...
\begin{defn}(Filter)
Let $\{x_t\}$ be a covariance-stationary process and let $\{h_j\}$ be an
absolutely summable sequence.  Then we can ``treat'' the data before
analysis by applying a function $h(\,\cdot\,)$ called a \emph{filter}
that produces a new, filtered series $\{y_t\}$ defined
\begin{align*}
  y_t &= h(L)x_t \\
  \text{where}\quad
  h(z) &= \sum_{j=-\infty}^\infty h_j z^j
\end{align*}
...
\begin{rmk}
Really almost every ARMA process we've dealt with has been a
``filtered series'' in the sense that the sequence of white noise shocks
$\{\varepsilon_t\}$ are covariance stationary and, if we can write the
ARMA as $y_t=\phi(L)^{-1}\theta(L)\varepsilon_t$, then $y_t$ is
given by applying the filter $\phi(L)^{-1}\theta(L)$ to the covariance
stationary process $\{\varepsilon_t\}$.
\end{rmk}
...
\begin{enumerate}[label=(\roman*)]
  \item Only $\alpha$ and $\beta$ are random. If you know them, you know
    \emph{exactly} how the process will evolve over time.
  \item The process is cyclic. For any integer $k$
    \begin{align*}
      y_{t+2\pi k/\lambda}
  ...
  \item
    The process has frequency $\lambda/(2\pi)$, i.e.\ completes
    $\lambda/(2\pi)$ cycles between any $t$ and $t+1$.
    Equivalently, the process has period $2\pi/\lambda$, i.e.\ it takes
    $2\pi/\lambda$ units of $t$ to complete one cycle. So if $t$ indexes
    days, months, or years, the process takes $2\pi/\lambda$ days,
    months, or years (respectively) to complete a single cycle.
  ...
  \item The process has mean zero $\E[y_t]=0$ (clear by inspection) and
    second moments:
    \begin{align*}
      \gamma(k) = \E[y_ty_{t+k}]
        &=
  ...
  \item
    ...
    of $y_t$ to pin down. Then compute the population regression
    coefficients of $y_t$ on $(y_{t-1}\;y_{t-2})$ to derive the optimal
    linear predictor of $y_t$ given $(y_{t-1}\;y_{t-2})$. They turn out
    to be $(2\cos(\lambda)\; -1)$, suggesting that we can write
    \begin{align*}
...
Any mean-zero stationary process $\{y_t\}$ with absolutely summable
autocovariances can be represented as
\begin{align}
  y_t &=
...
for all $\lambda,\lambda' \in [0,\pi)$ and also
\begin{align*}
    \E[\alpha(\lambda)^2] = \E[\beta(\lambda)^2] = 2f(\lambda)
    \qquad \forall \lambda,\lambda' \in [0,\pi)
\end{align*}
where $f(\lambda):[0,\pi]\ra \R$ is called the
\emph{spectral density function} and captures the contribution to the
total variance of $y_t$ from cycles/oscillations at each frequency
$\lambda\in[0,\pi)$.

Note that $\alpha(\lambda)$ and $\beta(\lambda)$ are
\emph{random functions}.
...
also to corresponding $\alpha(\lambda)$ and $\beta(\lambda)$ function
realizations that Representation~\ref{spectralrep} true.
\end{thm}
%\begin{rmk}
...
We can alternatively expression $y_t$ with spectral representation as in
Equation~\ref{spectralrep} by enlarging the domain of $\alpha$ and
$\beta$ from $[0,\pi)$ to $[-\pi,\pi)$ by defining, for all
$\lambda \in[0,\pi)$
\begin{align*}
...
Given the autocovariance function  of a process
\begin{align*}
...
Suppose that $\{y_t\}$ is a stationary and ergodic process with
absolutely summable autocovariances and $\E[y_t]=\mu$. Then we define
the \emph{long-run variance} as
\begin{align}
...
$T\ra\infty$ will give the desired expression, since $(T-|j|)/T \ra 1$
for all $j$ and absolutely summable autocovariances ensures
convergence.
\end{proof}
...
Given stationary and ergodic time series $\{y_t\}$ with $\E[y_t]=\mu$,
the long-run variance $S$ as defined in Expression~\ref{clt:lrvar}
satisfies
\begin{align*}
...
This section organizes divides into two themes: \emph{assessing} and
\emph{constructing} estimators, which are functions that map the
...
are general frameworks designed to produce estimators.
...
values from the real, true, written-in-God's-notebook parameter vector
$\bstheta$.  To form our estimator $\bshattheta$, we typically use
sample data $\{\bsy_n\}\nN$, where $\bsy_n\in\R^k$ refers to the $n$th
observation. We will often use the shorthand $\bsy_{1:N}$ to refer
to the whole sample $\{\bsy_n\}\nN$.
...
by implication, the true likelihood), though we will assume that we
know the functional form $f(\bsy_n|\bstildetheta)$, called simply the
\emph{likelihood} function (ditching the ``true'' modifier).
We sometimes restrict $\bstildetheta$ to lie in some subspace
$\bsTheta\subseteq\R^p$ to implement nonnegativity constraints or
other parameter resrictions (but in principle, $\bsTheta$ could be
all of $\R^p$). Therefore, we will think of
...
$\bshattheta = \bsg(\bsy_{1:N})$ that maps $N$ observations into a guess
for $\bstheta$.
Given a sample realization $\bsy_{1:N}$, we can plug in the values and
compute a number called an \emph{estimate}. In an abuse of notation, we
will also use $\bshattheta$ to denote an estimate, but it should always
be understood that any estimate depends on the sample used to compute
that estimate.

Alternatively, we can instead treat the sample $\bsy_{1:N}$ as a
collection of random variables (with some joint distribution) and study
the statistical properties of the estimator \emph{ex ante}, before the
sample is realized. In this way, we treat the estimator $\bshattheta$ as
\emph{itself} a random variable with some sampling distribution,
expectation, variance, etc.

This section is concerned more with estimat\emph{ors} than
estimat\emph{es}. We don't particularly care about plugging a realized
sample into a function to generate some number/estimate. We care more
about finding good functions/mappings (estimators) from sample
$\bsy_{1:N}$ to estimates in the first place. But to understand what
makes a ``good'' estimator, we need to study its distribution and large
sample properties. That is the concern of the rest of this section. We
begin by examining the topics of bias, consistency, loss, and risk which
help identify good estimators. We then put these concepts to work by
looking at general frameworks for estimation.
\end{defn}
...
\begin{align*}
  R(\bshattheta,\bstheta) = \E_{\bsy|\bstheta}[L(\bshattheta,\bstheta)]
\end{align*}
...
\begin{align*}
  ...
  = \E_{\bstheta}[ R(\bshattheta,\bstheta)]
\end{align*}
...
\clearpage
\subsubsection{Bias and Consistency}

Bias and consistency are two of the most important concepts when
...
and its variance:
\begin{align*}
  \E[(\bshattheta-\bstheta)^2]
  =\Var(\bshattheta) + \left(\E[\bshattheta-\bstheta]\right)^2
\end{align*}
\end{prop}


\begin{defn}(Consistency)
An estimator $\bshattheta_N$ using $N$ sample points is
\emph{consistent} if $\bshattheta_N\pto \bstheta$ as $N\ra\infty$.
\end{defn}

\begin{defn}(Strongly Consistent)
An estimator $\bshattheta_N$ using $N$ sample points is
\emph{strongly consistent} if $\bshattheta_N\asto \bstheta$.
\end{defn}
...
\end{defn}
\begin{rmk}
In general, information $\calI(\bstheta)$ is \emph{not} a function. It
is always evaluated at the true $\bstheta$. It only made sense to define
a score \emph{function} which depends upon $\bstildetheta$ because we
...
when the above is evaluated at the true $\bstheta$.
\end{rmk}
...
\end{prop}
\begin{proof}
We prove the result for the scalar case.
...
\begin{align*}
  ...
  \calI(\bstheta)^{-1}
\end{align*}
i.e.\ the difference $\Var(\bshattheta)-I(\bstheta)^{-1}$ is a postive
semidefinite matrix.
\end{thm}
\begin{proof}
...
\subsection{Estimation Frameworks}
...
\subsection{Method of Moments and GMM Defined}

A model and its assumptions typically imply certain \emph{moment
conditions} which state that some function $\bsf:\R^M\times\R^K \ra
\R^L$ of random variable $\bsw_n\in\R^M$ and true parameter vector
$\bsbeta \in\R^K$ must equal (without loss of generality) the constant
zero vector:
\begin{align}
  \label{populationMomentConditions}
  \bsg(\bsbeta) := \E[\bsg_n(\bsbeta) ]
  = \E[\bsf(\bsw_n,\bsbeta)] = \bso
\end{align}
where $\bsg:\R^K\ra \R^L$. Notice that we introduced the shorthand
notation $\bsg_n(\bsb)=\bsf(\bsw_n,\bsb)$.
Our goal is to estimate $\bsbeta$. Quite sensibly, we can estimate
parameters by drawing a sample $\{\bsw_n\}\nN$, using them to construct
sample observations $\bsg_n(\bsb)$, and then forming the sample analog
$\bsg_N$ of the moment conditions
\begin{align}
...
\end{align}
This is a function of parameter vector $\bsb$. We will naturally choose
our estimator to be the parameter vector that sets this sample analog
equal to zero or ``close'' to zero.

\begin{defn}(Method of Moments Estimator)
When the dimensionality of $\bsbeta$ equals that of $\bsg(\bsbeta)$
(when there are as many parameters as moment conditions $K=L$), we can
...
the solution $\bshatbeta$ to the following system of
equations:
\begin{align*}
  \bsg_N(\bshatbeta) &= \bso
  \qquad\text{where}\quad
  \bsg_N,\bshatbeta \in\R^K
\end{align*}
\end{defn}
...
\label{defn:gmm}
When there are more moment conditions than parameters, i.e.\ when $L>K$
so that the model is \emph{overidentified}, we cannot generally set the
sample analog of the moment conditions $\bsg_N(\bsb)$ to zero
\emph{exactly} using some $K$-dimensional $\bsb$.
Therefore, we instead look for the parameter vector that gets the
squared-weighted elements of vector $\bsg_N(\bsb)$ jointly as
...
\begin{align}
  ...
  \notag
\end{align}
where $\bshatbeta(\bshatW)$ is called
...
employing a selection matrix $\bshata\in \R^{K\times L}$ to
...
we solve this modified, but exactly-identified variation on the original
moment conditions:
\begin{align}
  \bshata \; \bsg_N(\bshatbeta) &= \bso
  \qquad\text{where}\quad
  \bshatbeta \in\R^K\quad \bshata \in \R^{L\times K}
  \label{eq:bhatmina}
\end{align}
We see that $\bshata\,\bsg_N(\bshatb)$ is like a new set of moment
conditions, but with the dimensionality reduced from $L>K$ to $K$ so
that we \emph{can} set this to zero for some $\bshatbeta\in\R^K$.
...
out the moments in $\bsg_N(\bshatb)$ (or form linear combinations of
moments) that we think are most important and definitely want to force
to zero. But in general, it really can be any arbitrary matrix in
$R^{L\times K}$.

Lastly, just to tie things together, this is actually equivalent to
Definition~\ref{defn:gmm} if you choose $\bshata$ based on the first
order conditions to the minimization problem defined by
Equation~\ref{eq:bhatmin}:
\begin{align}
  \bso
  =
  \underbrace{\frac{\partial \bsg_N'}{\partial \bsb} \bshatW}_{\bshata}
  \;\bsg_N(\bsb)
  \label{eq:equivrep}
\end{align}
\end{defn}
...
In some cases, our goal is to obtain not just consistent estimators, but
also \emph{efficient} GMM estimators, i.e.\ an estimator $\bshatbeta$
with the lowest possible asymptotic variance.
That typically presents the following chicken and egg problem:
\begin{enumerate}[label=(\roman*)]
  \item Obtaining the efficient GMM estimator, call it $\bshatbeta_2$,
    often requires using a specific/specially-chosen weighting matrix,
    call it $\bshatW_2$
  \item However, the weighting matrix $\bshatW_2$ that
    delivers the the efficient estimator is often a function of the true
    parameter vector $\bsbeta$. Since we don't know the true parameter
    vector, we approximate it by a consistent estimate called
    $\bshatbeta_1$.
\end{enumerate}
That's the problem: (i) requires $\bshatW_2$ to get efficient
$\bshatbeta_2$, while (ii) requires some consistent estimate
$\bshatbeta_1$ to get the appropriate $\bshatW_2$.
For this reason, efficient estimation is typically carried out in two
steps:
\begin{enumerate}
  \item Choose some arbitrary $\bshatW_1$ ($\bsI$ often works) and solve
    Expression~\ref{eq:bhatmin} for a preliminary ``inefficient'', but
    consistent estimator $\bshatbeta_1$.
  \item Use $\bshatbeta_1$ to compute the second-stage efficient
    weighting matrix $\bshatW_2$, and then use this to solve
    Expression~\ref{eq:bhatmin} again for the now efficient GMM
    estimator $\bshatbeta_2$.
\end{enumerate}
This is all very general, but there will be specific examples of this
procedure below.
\end{defn}
\begin{rmk}
...
\clearpage
\subsection{Example: OLS as a GMM Estimator}
...
\begin{align*}
  \bsg_N(\bsb) &= \frac{1}{N}\sumnN (y_n-\bsx_n'\bsb)\bsx_n
...
\clearpage
\section{Estimating Spectral Densities and Long-Run Variance}

\subsection{Sample Periodogram}

\begin{defn}(Discrete Fourier Transform)
Consider $\{y_t\}_{t=1}^T$ and assume $T$ is odd. The discrete fourier
transform maps the series into the mean and $T-1$ trigonometrically
weighted averages:
\begin{align*}
  \hat{\mu} &= \frac{1}{T}\sumtT y_t \\
  Z^{\cos}_\ell &= \frac{\sqrt{2}}{\sqrt{T}}
    \sumtT \cos(\lambda_\ell (t-1))y_t
  \qquad \ell=1,\ldots,T-1
  \\
  Z^{\sin}_\ell &= \frac{\sqrt{2}}{\sqrt{T}}
    \sumtT \sin(\lambda_\ell (t-1))y_t
  \qquad \ell=1,\ldots,T-1
\end{align*}
for $\lambda_\ell = (2\pi \ell)/T$ where, again, $\ell=1,\ldots,T-1$.
\end{defn}

\begin{prop}
For any mean-zero stationary series with absolutely summable
autocovariances, the pairwise \emph{correlations} between the $T$ random
variables that are constructed in the DFT converge to zero
asymptotically:
\begin{align*}
  ...
  \pto 0
\end{align*}
i.e.\ the random variables are asymptotically uncorrelated.
Moreover, their variance converges to the spectral density:
\begin{align*}
    \sup_{\ell\leq (T-1)/2}
    \bigg\lvert\;
      \E[(Z^{\cos}_\ell)^2]
...
\begin{defn}
We define the \emph{periodogram} at the Fourier frequencies
$\lambda_\ell = (2\pi\ell)/T$:
\begin{align*}
  I_T(\lambda_\ell) =
  \frac{(Z^{\cos}_\ell)^2 +(Z^{\sin}_\ell)^2}{2}
  \qquad \ell = 1,\ldots,T-1
\end{align*}
where from the previous proposition
\begin{align*}
  \E[I_T(\lambda_\ell)] = 2\pi f(\lambda_\ell)
  \qquad \ell = 1,\ldots,T-1
\end{align*}
\end{defn}

\begin{prop}\emph{(Whittle Likelihood)}
If $\{y_t\}$ is a Gaussian process, then $Z^{\cos}_\ell$ and
$Z^{\sin}_\ell$ will be Gaussian such that
\begin{align*}
  Z_\ell^{\cos} &\sim N(0,2\pi f(\lambda_\ell)) \\
  Z_\ell^{\sin} &\sim N(0,2\pi f(\lambda_\ell))
\end{align*}
for $\ell=1,\ldots,T-1$. Now let $f(\lambda;\theta)$ denote the the
spectral density function at frequency $\lambda$ as a function of
parameters $\theta$. Then we can write the log-likelihood of the model
as
\begin{align*}
  \frac{T}{2}\log(2\pi)
  - \sum_{\ell=1}^{(T-1)/2} \log\big(2\pi f(\lambda_\ell;\theta)\big)
  - \sum_{\ell=1}^{(T-1)/2}
...
\clearpage
\subsection{HAC Estimators for the Long-Run Variance}
...
$\{\hat{\gamma}(j)\}$ and then construct
\begin{align*}
...
\end{align*}
where $\hat{\rho}$ is the coefficient from an OLS regression of $y_t-\mu$ on
$y_{t-1}-\mu$ and $\hat{S}_u$ is a nonparametric estimate (probably a
Newey-West estimator) of the long-run variance of the residuals.

The idea is that the first stage OLS regression of $y_t-\mu$ on
$y_{t-1}-\mu$ will remove a lot of the dependence  while estimating only
one parameter $\hat{\rho}$. As a result, the nonparametric
estimate of the residual long-run variance
...
for long langs won't contribute as much---the first state removed lots of
the autocorrelation already.
\end{defn}
\begin{proof}
...
Of course, this suppose that we know $\psi(L)$. In general we don't, so
we have to estimate it and do feasible GLS. Therefore, we can do the
following:
\begin{enumerate}[label=(\roman*)]
  \item Run the regression $y_t=x_t'\beta +u_t$ to get the vector of
    residuals $\hat{u}_t$.
...
\end{align*}
However, we need $\E[\tilde{x}_t\varepsilon_t]$ for consistency of the
...
\begin{enumerate}
  ...
  \item \emph{Martingale Difference Sequence $\bsx_n\varepsilon_n$}:
    ...
    martingale difference sequence relative to some filtration (the
    natural filtration will do just fine), i.e.\
    \begin{align*}
...
\clearpage
\subsection{Estimation of AR($p$) Model}
...
Regressors in $\bsx_n$ that are also instruments in $\bsz_n$ are called
\emph{predetermined}, while the other elements of $\bsx_n$
are called \emph{endogenous}.
Assumptions:
\begin{enumerate}
  \item \emph{Linearity}: $y_n = \bsx_n' \bsbeta + \varepsilon_n$
  ...
  \item \emph{Rank \& Order Conditions for Identification}:
    ...
    First, this \emph{rank condition} ensures that the instruments and
    regressors are correlated, otherwise the IV estimator is undefined.
    Second, linearity and Assumption 3 state
    \begin{align*}
    ...
    \end{align*}
    i.e.\ true $\bsbeta$ is \emph{a particular} solution of the above
    $L$ equations in $K$ unknowns. The \emph{rank condition}
    $\rank(\E[\bsz_n\bsx_n'])=K$ then also ensures that true $\bsbeta$
    is the \emph{only} solution, i.e.\ that $\bsbeta$ is identified.
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Use $\bshatbeta(\bsW_1)$ to compute residuals
    ...
    \end{alignat*}
\end{enumerate}
As in the one-stage procedure, \emph{both} $\bshatbeta(\bshatW_1)$ and
$\bshatbeta(\bshatW_2)$ consistently estimate $\bsbeta$, though
$\bshatbeta(\bshatW_2)$ has lower variance.
Using $\bshatW_2\pto \E[\varepsilon_n^2\bsz_n\bsz_n']^{-1}$,
if we sub this into the formulas in the previous subsection, the
asymptotic distribution of our estimator dramatically simplifies to
\begin{align*}
  ...
  \right)
\end{align*}
To form a consistent estimator for the asymptotice variance, we simply
take
\begin{align*}
  &
  \left[
    \left(
    \frac{1}{N}\sumnN \bsx_n\bsz_n'
    \right)
    \left(
    \frac{1}{N}
    \sumnN e_n \bsz_n\bsz_n'
    \right)^{-1}
    \left(
    \frac{1}{N}\sumnN \bsx_n\bsz_n'
    \right)
  ...
  &\qquad\qquad =
  N \cdot \left[
      (\bsX'\bsZ)\left(\sumnN e_n\bsz_n\bsz_n'\right)(\bsX'\bsZ)
    \right]^{-1}
  \pto
    \big(
    \E[\bsx_n\bsz_n']
...
\begin{align*}
  e_n &= y_n - \bsx_n'\bshatbeta(\bshatW)
\end{align*}
where $\bshatbeta(\bshatW)$ is the GMM estimator given some weighting
matrix $\bshatW$ with probability limit equal to a symmetric positive
definite matrix (since that always delivers a consistent estimate of
$\bsbeta$, and we need a consistent estimator to get appropriate
estimates of the residuals).
Of course, either $\bshatW_1$ or $\bshatW_2$ would work.
\end{defn}
...
Let $\bsSigma$ denote the asymptotic variance $\bsP\bsS\bsP'$ derived
above in Proposition~\ref{prop:gmmasymptotics}, and let $\bshatSigma$
represent the corresponding consistent estimator of that asymptotic
variance. Then these estimators can be used to conduct the following
tests:
\begin{enumerate}
  \item \emph{Robust $t$-Ratio for Testing Coefficients}: We can
    test the null hypothesis $H_0:\beta_k = \beta_k^0$ (where $\beta_k$
  ...
  \item \emph{``LR'' Test for Testing Non-Linear Hypotheses}:
    ...
    $J(\bsb, \bshatW) := N\cdot \bsg_N(\bsb)' \bshatW \bsg_N(\bsb)$
    and $\bshatS$ is a consistent estimator
    of $\E[\varepsilon_n^2\bsz_n\bsz_n']$, the inverse of the efficient
    weighting matrix. Then
    \begin{align*}
      LR := J(\bsbarbeta, \bshatS^{-1}) - J(\bshatbeta, \bshatS^{-1})
      \quad \dto \quad \Chi^2_C
...
\begin{itemize}
  \item This test \emph{only} works with an efficient GMM weighting
    matrix such that
    $\bshatS^{-1}\pto \E[\varepsilon_n^2\bsz_n\bsz_n']^{-1}$, otherwise,
    the LR is not asymptotically $\Chi^2$. In contrast, Wald test (3)
    works for any weighting matrix $\bshatW$, including inefficient
    ones.

  \item The same consistent estimate $\bshatS^{-1}$ for
    $\E[\varepsilon_n\bsz_n\bsz_n']$ should be used for both
    ...
    \begin{enumerate}[label=(\roman*)]
      ...
      \item Use consistent $\bshatbeta(\bshatW)$ to compute the
        ...
        $\E[\varepsilon_n\bsz_n\bsz_n']^{-1}$, which we denote by
        $\bshatS^{-1}$.

      \item Use $\bshatS^{-1}$ to solve Problem~\ref{gmm:lrunrestricted}
        for the efficient unrestricted GMM estimator $\bshatbeta$.

      \item Use that same $\bshatS^{-1}$ to solve
        Problem~\ref{gmm:lrrestricted} for efficient restricted
        GMM estimator $\bsbarbeta$.
    \end{enumerate}
    Notice that we didn't resolve for a new $\bshatS^{-1}$ before Step
    ...
    the same consistent $\bshatS^{-1}$ for both minimization problems.

    While not a problem asymptotically, using different consistent
    estimates of $\bshatS^{-1}$ in solving
    ...
    the same $\bshatS^{-1}$ in both avoids this problem.
\end{itemize}
\end{rmk}
...
to all the results in Subsection~\ref{subsec:EndogenousRegressorGMM}.
But that then raises the question ``When is it a \emph{good} idea to use
that weighting matrix and thus use the 2SLS estimator?'' That gives us
...
that, i.e.\
\begin{align}
  \text{\emph{Conditional homoskedasticity:}}\quad
  ...
  \label{2SLSerror}
\end{align}
the efficient GMM estimator reduces to the 2SLS estimator.
