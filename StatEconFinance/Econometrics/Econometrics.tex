...
\end{align}
where $g:\R^K\ra \R^L$. Our goal is to estimate $\bsbeta$.
Quite sensibly, we can estimate parameters by drawing a sample
$\{\bsw_n\}\nN$ of the random variable, constructing a sample analog
...
equal to zero or ``close'' to zero, with these two notions leading us to
...
Definition~\ref{defn:gmm} if you set
\begin{align}
  \bshata =
  \frac{\partial \bsg'_N}{\partial \bsb}
  \bshatW
  \label{eq:equivrep}
\end{align}
This follows from the first order conditions to the minimization defined
by Equation~\ref{eq:bhatmin}:
\begin{align*}
  \bso
  =
