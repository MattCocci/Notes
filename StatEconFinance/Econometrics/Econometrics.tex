...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Definition of $\sigma$-algebra and properties
    \item Definition of a measure $\mu$ and properties:
      finite additivity, monotonicity, subadditivity, complete
    \item Results for measures, continuous from above and below
    \item Different collections than $\sigma$-algebras: Monotone class,
      Dynkin system---both weaker $\sigma$-algebra
    \item Intersections of collections of a certain type are also
      collections of that type. Define ``smallest''
    \item
      Generally, we want to prove that a property holds for all sets in
      a $\sigma$-algebra. To do so, show the property holds in a smaller
      set of things. Show this smaller set of things is a monotone
      class, then apply the monotone class theorem. Or show this smaller
      set of things is a Dynkin system and apply the $\pi-\lambda$
      lemma.
    ...
    \item
      ...
      \begin{enumerate}
        \item Lemma for MCT:
          algebra $\sA=\sigma(\sA)$
          if and only if $\sA=\sM(\sA)$.
          In words, algebra $\sA$ is a $\sigma$-algebra if and only if it
          is a monotone class
        \item MCTI: If $\sA$ is an algebra then
          $\sM(\sA)=\sigma(\sA)$. Use principle of appropriate sets
        \item Corollary to MCTI: $P$ and $Q$ are prob measures on
          $\sigma(\sA)$ that agree on algebra $\sA$, then agree on
          $\sigma(\sA)$.

          To prove this, define
          \begin{align*}
            \sN = \{A\in \sigma(\sA)|P[A]=Q[A]\}
          \end{align*}
          We have the following results
          \begin{enumerate}
            \item $\sN$ is a monotone class.
              Easy to see
            \item $\sN\supset \sA$. Also clear
          \end{enumerate}
          Then clearly $\sN\supset \sM(\sA)$.
          By (b), we have that $\sM(\sA)=\sigma(\sA)$, hence
          $\sN\supset \sigma(\sA)$. Therefore, done

        \item Lemma for MCTII: Dynkin system $\sD$ closed under finite
          intersections is a $\sigma$-algebra.
          In other words, $\sD$ closed under finite intersections
          implies $\sD=\sigma(\sD)$.
        \item MCTII: If $\sE$ nonempty \& closed under finite
          intersections, $\delta(\sE)=\sigma(\sE)$.
