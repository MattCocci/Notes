...
\begin{itemize}
  \item \emph{Density} of $\mu_2$ with respect to the dominating measure
    $\mu_1$:
    This generalizes
    %(that also makes explicit mention of the measures in question)
...
%This subsection defines and derives properties about the
%Markov Semigroup $\{P_t\}_{t\geq 0}$ and infinitesimal generator of
%time-homogeneous
%general time \emph{homogeneous} Markov
%processes. In
%In the next, we consider the special case of \emph{diffusion}
%processes, where all results of this subsection also apply.
...
%$\E_t[df(x_t)]$, and as is clear from the expression for $\calL$,
%$\frac{1}{dt}\E[df(x_t)]=\partial f(x_t)/\partial t+\calL f(x_t)$.
\end{defn}
\begin{proof}
...
%To motivate the Ito Integral, recall the Riemann-Stieltjes integral.
%In particular, suppose $f$ is Riemann-Stieltjes integrable with respect
%to weighting function $\alpha$. Then
%\begin{align}
  %%\int f \; d\alpha
  %%=
...
%Ito integration is a generalization that replaces \emph{deterministic}
%weighting function $\alpha(t)$ with \emph{stochastic} weighting function
%$W(t)=W_t$, standard Brownian motion. Note that this implies the
%computed value of the integral is \emph{stochastic} (a random variable)
%since the weights assigned to time increments ($W_{t_n}-W_{t_{n-1}}$ for
%$[t_{n-1},t_n]$) vary across states of nature.  But we construct the Ito
%Integral just like the Riemann integral: start with an approximating sum
%and then take limits.
%We just need to first introduce some definitions that can be used to put
%regularity conditions on the functions that we integrate.


%\begin{defn}(Adapted to $X_t$)
%Stochastic process $\{V_t\}_{t\geq 0}$ is \emph{adapted} to stochastic
%process $\{X_t\}_{t\geq 0}$ if $V_{s}$ and $X_{t}-X_{s}$ are independent
%RVs for \emph{all} $s,t$ such that $s<t$.
...
%\begin{rmk}
%In words, this definition requires that $V_s$ does not ``look ahead''
%and incorporate information about $X_{t}-X_s$.
%Though $V_s$ can still depend on \emph{past} $X_{s'}$ for $s'\leq s$ in
%any arbitrary way, this definition says that $V_s$ cannot depend on
%\emph{future} values of stochastic process $X_s$.
...
%information about \emph{future} $X_t$ (for any $t'>t$) to inform our
...
%\begin{defn}(Ito Process, Ito Diffusion)
%Stochastic process $\{X_t\}_{t\geq 0}$ is an \emph{Ito Process} if it
...
%If drift and diffusion coefficients $\mu(\,\cdot\,)$ and
%$\sigma(\,\cdot\,)$ do \emph{not} depend upon time, $X_t$ is called an
%\emph{Ito Diffusion} (or just \emph{diffusion}).
%%Generalized and Standard Brownian
...
%to say that ``Solution $\{X_t\}_{t\geq 0}$ is an Ito Process satisfying
%Equation~\ref{itodiffusion}.''
%Both are equivalent and encode the same information.
%\end{defn}

%\begin{defn}(Solutions to SDEs)
%Given any SDE of the form in Equation~\ref{itosde}, all we know is that
%the solution $X_t$ \emph{satisfies} Equation~\ref{itodiffusion} by
%definition. But that doesn't immediately give a solution since $X_t$ is
%on both the LHS and the RHS of Equation~\ref{itosde}.
%To truly have a solution for $X_t$, we must write $X_t$ as a function of
...
\begin{itemize}
  ...
  \item $p(\bsy|\bstildetheta)$:
    ...
    and likelihood.
    %If $\bsy=\bsy_{1:N}$, then this is a joint density for the entire
    %sample, sometimes called the \emph{joint likelihood function}.
\end{itemize}
...
%Again, some notation. This section considers likelihood function
%$p(\bsy|\bstildetheta)$, which is a function of
%$\bstildetheta\in\bsTheta$.
%There is also some true parameter value $\bstheta$ and corresponding
%true likelihood $p(\bsy|\bstheta)$.
%Note $\bsy$ could just be a single observation \emph{or} a sample
%$\bsy=\{\bsy_n\}\nN$ (in which case $p(\bsy|\bstildetheta)$ is a
%\emph{joint} likelihood function for the entire sample).
%Regardless, all results of this section hold for either case.

%Since data is generated according to a \emph{true likelihood} function
%$p(\bsy|\bstheta)$, expectations involving functions of $\bsy$ should be
%computed using the true likelihood function $p(\bsy|\bstheta)$, which is
%why we will explicitly write $\E_{\bsy|\bstheta}[\;\cdot\;]$ whenever we
%have to compute expectations below.
...
%the definition of variance and
Proposition~\ref{prop:score0}. We will prove both those propositions
below.
\end{defn}
%\begin{rmk}
%Like the score, the information is evaluated at \emph{true} $\bstheta$.
%And recalling the distinction between ``score'' and ``score function,''
%I could have similarly defined an ``information \emph{function}.''
%But that's not really helpful.
%It only made sense to define a score \emph{function} (which depends upon
%$\bstildetheta$) because we will want to set that to zero in MLE
%estimation---doing so sets sets the first derivative of the
%log-likelihood to zero so that we maximize the log-likelihood. It's
%perfectly natural to think of that first-derivative function. However,
%there's no similar motivation or justification for defining an
%information function.
%So this definition of just the information (evaluated at true
%$\bstheta$) suffices.
%\end{rmk}
...
\begin{align*}
  ...
  %\frac{\partial}{\partial\bstildetheta}
  %\left[
    %\int p(\bsy_n|\sF_{n-1},\bstildetheta) \; d\bsy_n
  %\right] \\
  %\iff\qquad
  \bso
  &=
...
%So by Slutsky, we have the result.
%\begin{align*}
  %\sqrt{N}(\bshattheta-\bstheta)
  %\quad\dto\quad
  %\calN\big(0,
...
\begin{align*}
  ...
  %}_{=\Var[\varepsilon_nf'(\bsx_n,\bstheta)]}
  \big)
\end{align*}
...
%\begin{enumerate}[label=(\roman*)]
  %\item
    ...
    %And the true $\bstheta$ is a maximizer of
    %$\E_{\bsy|\bstheta}\big[\ln p(\bsy_n|\bstildetheta)\big]$ iff
    %\begin{align*}
    ...
    %\begin{align*}
      ...
      %\qquad\forall \bsdelta
    %\end{align*}
    %But that is immediate if we apply Jensen's inequality to the LHS to
    %get:
    %\begin{align*}
...
\begin{enumerate}[label=\emph{(\roman*)}]
  ...
  \item
    ...
    %Expressed in more compact notation:
    %\begin{align*}
      %\sup_{\theta\in\Theta_0}
      %\E_\theta[\varphi_\Lambda(y)]
      %\leq\alpha
      %\quad\implies\quad
      %\E_1[\varphi(y)]
      %\leq
      %\E_1[\varphi_\Lambda(y)]
      %\quad
      %\forall
      %\varphi(y)
      %\;\; \text{s.t.}\;\;
...
%\clearpage
%\begin{defn}
%{(Generalized Likelihood Ratio Test for Composite Hypotheses)}
%Suppose we want to test the following simple null and alternative
%hypotheses about the unknown parameter $\bstheta$ in the parametric
%model/likelihood $p(\bsy|\bstheta)$ that generates data $\bsy$:
%\begin{align*}
...
%\emph{generalized likelihood ratio test}:
%\begin{align*}
  %\varphi^*(\bsy)
  %=
  %\begin{cases}
  ...
  %}
%\end{align*}
%where, again, $\lambda,q$ are constants chosen to satisfy the
%$\alpha$-level constraint:
%\begin{align}
  %\alpha
  %= \int \varphi^*(\bsy) p(\bsy|\bstheta_0)\; d\bsy
  %\qquad\text{where}\quad
  %\bstheta_0:=
  %\argmax_{\bstildetheta \in A_0}p(\bsy|\bstildetheta)
%\end{align}
%\end{defn}
...
\begin{enumerate}
  ...
  \item \emph{Asymptotic Normality}: Under assumptions 1-4
    \begin{align*}
      ...
      \big)
      %N(0,\E[\bsx_n\bsx_n']^{-1}\;\E[\varepsilon^2_n\bsx_n\bsx_n']\;\E[\bsx_n\bsx_n']^{-1})
    \end{align*}
...
\begin{enumerate}
  \item \emph{Robust $t$-Ratio for Testing Coefficients}: We can
    ...
    %(where $\bsbeta_k$ is the $k$th element of $\bsbeta$)
    by constructing the
    \emph{robust $t$-ratio}:
    \begin{align*}
      t_k =
...
%It may not be strictly optimal, but Expression~\ref{Fchow} at least
...
\begin{align*}
  ...
  %&=
  %\left[
  %\frac{1}{\tau}\sum_{t=1}^\tau (\mu+u_t)
  %\right]
  %-
  %\left[
  %\frac{1}{T-\tau}\sum_{t=\tau+1}^T (\mu+u_t)
  %\right]
  %\\
  &=
  \left[
