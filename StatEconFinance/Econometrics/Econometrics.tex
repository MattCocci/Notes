...
Within the state space framework detailed above, we now face the task of
computing the model likelihood and estimating the sequence of (possibly
unobserved) states. To do so, we typically use the Kalman filter, a
quick recursive procedure for doing just that.
...
Throughout this entire section, we take $\bstheta$ as given, implying
that the matrices defined in Equation~\ref{ste} and Equation~\ref{moe}
are entirely \emph{known}. The Kalman Filter algorithm offers a way to
compute the likelihood $\calL(\bstheta)=p(\bsy_{1:T}|\bstheta)$
and estimates of the states $\{\bss_t\}_{t=1}^T$ \emph{given}
parameter vector $\bstheta$. But to estimate these
parameters (which we will discuss in more detail below), just kick
$\bstheta$ around the parameter space, compute the likehood via the
Kalman Filter, and choose the parameters that delivered the highest
value as your maximum likelihood estimate. But more on that later.

\subsubsection{Outline of the Recursive Procedure}
\label{subsec:kfoutline}

Here's the basic Kalman Filter algorithm for any linear state space
model as written in Equations~\ref{ste} and~\ref{moe}, again taking
parameters $\bstheta$ and data $\bsy_{1:T}:=\{\bsy_t\}_{t=1}^T$ as
given. Throughout, I will use $\calI_{t}=\{\bsy_s\}_{s=1}^t$ to denote
the set of information available at time $t$.
\begin{enumerate}
\item
  \emph{Prior}:
  Start with a prior for the state vector at time $t$, denoted
  $p(\bss_t |\; \calI_{t-1})$.\footnote{%
...
\item
  ...
  ``filtering'' out the noise, to get $p(\bss_t |\; \calI_{t})$, the
...
\item Increment $t$ and return to step 1, taking predictive
  distribution $p(\bss_{t+1} |\; \calI_{t})$ as the prior.
...
\emph{variance} of $\{\bss_t\}$ over time. This is done because we often
assume normal errors $\bsvarepsilon_t$ and $\bseta_t$, in which case the
mean and variance pin down the successive distributions
{entirely}.\footnote{%
  This is a feature of multivariate normal random variables.
}
However, the procedure outlined above and in
the next subsection is ``General'' in that it frames the recursive
process in terms of the full \emph{distributions} for $\{\bss_t\}$ over
time---i.e. $p(\bss_t|\calI_t)$ rather than
$\E[\bss_t|\calI_t]$ and $\text{Var}(\bss_t|\calI_t)$.
...
assumption might not be as dumb as it seems.\footnote{%
  Hint: Regardless of the true error distributions, the Kalman Filter
  gives the best linear projection estimates of the states. It's kind of
  like when you know your data isn't normally distributed, but you still
  run regressions because it gives the best linear approximation to the
  CEF.
}
%For that discussion, follow a minor yet very important detour in two
%subsections.
...
Given a sequence of data points $\{\bsy_t\}_{t=1}^T$ our goal is to
estimate the sequence of states $\{\bss_t\}^T_{t=1}$. Of course, this
will all be probabilistic. Unless a state is mapped directly to an
observable (without measurement error or other shocks), we generally
won't be able to know with certainty the true value of the states. We'll
instead compute a sequence of probability distributions for the states
at each point in time. Again, we assume that the matrices of our linear
state space model defined by Equations~\ref{ste} and~\ref{moe} are
entirely known.

\begin{enumerate}
  \item \emph{Prior Step}:
    Standing at time $t-1$, suppose that we have a prior for $\bss_t$,
    given $\calI_{t-1}$,\footnote{%
      When $t-1=0$, this is a true unconditional prior since
      $\calI_{0}=\emptyset$, and we have no available observations as
      a basis for this prior. However, when $t>1$, the recursive Kalman
  ...
  \item \emph{Filtering Step}:
    Given prior $p(\bss_t|\calI_{t-1})$, we should then incorporate the
    information in our next observed data point $\bsy_t$ to update our
    estimate of the distribution for $\bss_t$.
    This step is called the ``Filtering Step'' because our combination
    of both prior and data $\bsy_t$ allows us to ``filter out'' the
    noise in our estimates for $\bss_t$.

    To do so, use Bayes' rule along with the fact that
    $\calI_{t} = \{\bsy_1, \ldots, \bsy_t\}=\calI_{t-1} \cup \bsy_t$:
    \begin{align*}
      p(\bss_t | \calI_{t}) =
    ...
    \end{align*}
    This doesn't look like we've gotten anywhere, but we can simplify a
    bit. By the setup and assumptions of Equations~\ref{ste}
    and~\ref{moe}, we know that past values $\bsy_1, \ldots, \bsy_{t-1}$
    have no bearing upon $\bsy_t$ if we already know the state,
    $\bss_t$. Hence $p(\bsy_t|\bss_t, \calI_{t-1}) = p(\bsy_t|\bss_t)$.
    In addition, since the denominator is just a normalizing constant,
    we can simplify to
    \begin{equation}
      \label{filt}
      \text{Filtering Distribution:} \quad
      p(\bss_t | \calI_{t}) \propto p(\bsy_t | \bss_t) \cdot
      p(\bss_t | \calI_{t-1})
    \end{equation}
    This is convenient since we already have
    $p(\bss_t | \calI_{t-1})$---it's our prior from above---while
    Equation~\ref{moe} plus the assumed distribution on the errors
    $\bseta_t$ entirely pins down $p(\bsy_t|\bss_t)$.
    %But no matter what distribution we assume for
    %the errors $\bseta_t$, our assumptions on their mean and variance in
    %Equation~\ref{moe} imply
    %\begin{align*}
      %\E[\bsy_t|\bss_t] = \bsD + \bsM \bss_t
      %\qquad
      %\text{Var}(\bsy_t|\bss_t) = \bsH
    %\end{align*}

  \item \emph{Forecast Step}: Given the filtering distribution, we
    ...
    \end{align*}
    But note that $p(\bss_{t+1}|\bss_t, \calI_{t})=p(\bss_{t+1}|\bss_t)$
    since according to the State Transition Equation, if we know
    $\bss_{t}$, observations $\bsy_1, \ldots, \bsy_t$ provide no
    additional information. This yields final expression
    \begin{align}
    ...
    $\bsvarepsilon_t$.
    %But no matter what distribution we assume for the errors, our
    %assumptions on their mean and variance in Equation~\ref{ste} imply
    %\begin{align*}
      %\E[\bss_{t+1}|\bss_t] = \bsC + \bsT\bss_t
      %\qquad
      %\text{Var}(\bss_{t+1}|\bss_t) = \bsR
    %\end{align*}

    Lastly, we can also get a forecasting distribution for the
    observables. In particular,
    \begin{align*}
      p(\bsy_{t+1}|\calI_t)
      &=
      \int p(\bsy_{t+1},\bss_{t+1}|\calI_t)\;d\bss_{t+1}
      =
      \int p(\bsy_{t+1}|\bss_{t+1},\calI_t)
        \cdot p(\bss_{t+1}|\calI_t)
      \;d\bss_{t+1}
    \end{align*}
    Now, $p(\bsy_{t+1}|\bss_{t+1},\calI_t)=p(\bsy_{t+1}|\bss_{t+1})$
    since $\calI_t=\{\bsy_1,\ldots,\bsy_t\}$ provides no further
    information if we know $\bss_{t+1}$. Hence,
    \begin{align}
      p(\bsy_{t+1}|\calI_t)
      &=
      \int p(\bsy_{t+1}|\bss_{t+1})
        \cdot p(\bss_{t+1}|\calI_t)
      \;d\bss_{t+1}
      \label{foredistobs}
    \end{align}
    The distribution $p(\bsy_{t+1}|\bss_{t+1})$ is entirely pinned down
    by the distribution we assume for the errors $\bseta_{t+1}$, while
    we also computed $p(\bss_{t+1}|\calI_t)$ above. Therefore, we have
    everything we need to comptue this object.


  \item \emph{Recursive Step}:
  ...
  \item \emph{Compute the Likelihood}:
    ...
    the likelihood $\calL(\bstheta)$. By successive conditioning and
    $\calI_t=\{\bsy_1,\ldots,\bsy_t\}$, we get
    \begin{align*}
      \calL(\bstheta)
      = p(\bsy_{1:T}|\bstheta)
      &= p(\bsy_T|\bsy_{1:T-1},\bstheta)\cdot p(\bsy_{1:T-1}|\bstheta)
      = \cdots \\
      %&= \prodtT p(\bsy_t|\bsy_{t-1},\ldots,\bsy_t,\bstheta)
      %\\
      \implies\quad
      \calL(\bstheta)
      &= \prodtT p(\bsy_t|\calI_{t-1},\bstheta)
    \end{align*}
    But we know the terms of the product; they are given by
    Expression~\ref{foredistobs} above.
\end{enumerate}
...
We now develop the Kalman Filter procedure in the special case of
Gaussian $\bsvarepsilon_t$ and $\bseta_t$ errors, which implies
Gaussian $\{\bss_t\}_{t=0}^T$.
...
But first, we introduce notation.
Letting $\bsmu$ and $\bsSigma$ represent the mean and variance, define
%\begin{alignat*}{8}
  %\bsmu_{t|t-1} &:=
  %\E_{t-1}[\bss_t] &&:=
  %\E[\bss_t|\;\calI_{t-1}]
...
\begin{enumerate}
  \item \emph{Prior Step}: Suppose that we have a Guassian prior for
    $s_t$, given $\calI_{t-1}$.\footnote{%
  ...
  \item \emph{Filtering Step}:
    Recall the goal is to compute filtering distribution
    $p(\bss_t|\calI_{t})=p(\bss_t|\bsy_t, \calI_{t-1})$.
    To do so, we will make use of the joint Gaussian distribution of
    $\bss_t$ and $\bsy_t$ (which follows from assumed Gaussian errors):
    \begin{align*}
      ...
      \right)
    \end{align*}
    which is true for some values $\bsmu_{i}$ and $\bsSigma_{jk}$ that
    we now pin down.
    \begin{itemize}
      \item $\bsmu_1$ and $\bsSigma_{11}$: These terms correspond to the
        mean and variance for $\bss_t |\; \calI_{t-1}$. But we
        defined them in the prior step above to be $\bsmu_{t|t-1}$ and
        $\bsSigma_{t|t-1}$, respectively.

      \item $\bsmu_2$ and $\bsSigma_{22}$: First, use Measurement
        ...
        \begin{align*}
          \bsmu_2 :=
            ...
            = \bsM\bsmu_{t|t-1}
        \end{align*}
        Now that we have the mean, let's get the variance of $\bsy_t$:
        \begin{alignat*}{2}
          \text{Equation~\ref{moe} and $\bss_t\perp\bseta_t$} &\qquad
            \bsSigma_{22}
            =
            \Var_{t-1}(\bsy_t)
            &&= \Var_{t-1}(\bsM\bss_t)
          ...
          %\text{Property of covariance matrices} &\qquad
            ...
            %+ \Var(\bseta_t) \\
          \text{From Equations~\ref{moe} and \ref{kalnot}} &\qquad
            &&= \bsM\bsSigma_{t|t-1} \bsM' + \bsH
        \end{alignat*}

      \item $\bsSigma_{12} = (\bsSigma_{21})^T$: Start with the
        definition
        \begin{align*}
          \bsSigma_{21}
          = \E_{t-1}\left[
    ...
    filtering distribution $p(\bss_t | \;\calI_{t})$ by computing the
    conditional multivariate normal formuls and using
    ...
    the scaled error for $\bsy_t$ relative to it's prior expectation
    $\bsM\bsmu_{t|t-1}$. In addition, the information provided by
    $\bsy_t$ allows us to reduce our the variance in our estimate of
    $\bss_t$ to $\bsSigma_{t|t}$ relative to the prior variance
    $\bsSigma_{t|t-1}$.

  \item
    \emph{Forecast Step}: Now that we have our filtered mean and
    ...
    \begin{alignat*}{3}
      ...
      && &= \bsT\bsSigma_{t|t}T' + \bsR\bsQ\bsR'
    \end{alignat*}
    A similarly straightforward application and writing-out of the
    measurement equation will yield the following forecast mean and
    variance for the observables:
    \begin{align*}
      \E[\bsy_{t+1}|\;\calI_{t}] &= \bsT\bsmu_{t|t} \\
      \Var(\bsy_{t+1}|\;\calI_{t})
        &= \bsM\left(\bsT\bsSigma_{t|t}\bsT' + \bsR\bsQ\bsR'\right)\bsM'
        + \bsH
    \end{align*}
\end{enumerate}
...
\clearpage
\subsection{Time-Varying State Space Models}
