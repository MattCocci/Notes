...
That was a lot of work, so what do these last two theorems buy us?
Well, we often want to prove that some property $P$ holds for all sets
in a $\sigma$-algebra $\sF$.
One option is to check that property $P$ holds for literally every
$A\in\sF$. \emph{Alternatively}, we can take a short cut by showing that
property $P$ holds on a much smaller collection $\sC\subsetneq \sF$
that simply \emph{generates} $\sF=\sigma(\sC)$ (so $\sF$ is an
``extension'' of $\sC$ in some sense), and apply an Monotone Convergence
Theorem to ensure that property $P$ holds on extension $\sF$.

\begin{defn}
(Cookbook Procedures for Working with Monotone Class Theorems)
Here's how to apply the monotone class theorems in practice.
Both make use of the principle of appropriate sets.
\begin{enumerate}[label=(\roman*)]
  \item
    Theorem~\ref{thm:mct-1}:
    Find an algebra $\sA$ such that $\sF=\sigma(\sA)$ and such that
    \begin{align*}
      \tilde{\sF} = \{A\in\sF \;|\; \text{Property $P$ holds}\}
      \subseteq \sF
    \end{align*}
    is a monotone class containing $\sA$.
    (Finding that $\sA$ is the hard part.)
    Then $\tilde{\sF}\supseteq m(\sA)$.
    Invoke Monotone Class Theorem~\ref{thm:mct-1} to assert that
    $m(\sA)=\sigma(\sA)=\sF$. Hence $\tilde{\sF}\supseteq\sF$, implying
    that $\sF=\tilde{\sF}$, i.e. property $P$ holds for all sets in
    $\sF$.

  \item
    Theorem~\ref{thm:mct-2}:
    Find a collection $\sE$ closed under finite intersections such
    that $\sF=\sigma(\sE)$ and such that
    \begin{align*}
      \tilde{\sF} = \{A\in\sF \;|\; \text{Property $P$ holds}\}
      \subseteq \sF
    \end{align*}
    is a Dynkin system containing $\sE$.
    (Finding that $\sE$ is the hard part.)
    Then $\tilde{\sF}\supseteq \delta(\sE)$.
    Invoke Monotone Class Theorem~\ref{thm:mct-2} to assert that
    $\delta(\sE)=\sigma(\sE)=\sF$. Hence $\tilde{\sF}\supseteq\sF$,
    implying that $\sF=\tilde{\sF}$, i.e. property $P$ holds for all
    sets in $\sF$.
\end{enumerate}
\end{defn}

\clearpage
We now apply the above cookbook approaches to derive two useful
corollaries.

\begin{cor}\emph{(Extension from MCT-I)}
Given algebra $\sA$ and measures $\mu_1,\mu_2$ on $\sigma(\sA)$
\begin{align*}
  \mu_1(A)=\mu_2(A)
  \quad \forall A\in\sA
  \quad\implies\quad
...
\end{cor}
\begin{proof}
We are applying cookbook approach (i), so define
\begin{align*}
  \sF = \sigma(\sA)
  \qquad
  \tilde{\sF} = \{A\in \sF\;|\;\mu_1(A)=\mu_2(A)\}
  \subseteq \sF
\end{align*}
It's easy to see that $\tilde{\sF}$ happens to be a monotone class that
contains $\sA$.
Hence $\tilde{\sF}\supseteq m(\sA)$.
So invoke Monotone Class Theorem~\ref{thm:mct-1} to get
$\tilde{\sF}=\sF$, i.e. $\mu_1(A)=\mu_2(A)$ for all $A\in\sF$.
\end{proof}

\begin{cor}\emph{(Extension to MCT-II)}
Given nonempty $\sE$ closed under finite intersections plus measures
$\mu_1$ and $\mu_2$ on $\sigma(\sE)$
\begin{align*}
  \mu_1(A)=\mu_2(A)
  \quad \forall A\in\sE
  \quad\implies\quad
...
\end{cor}
\begin{proof}
We are applying cookbook approach (ii), so define
\begin{align*}
  \sF = \sigma(\sE)
  \qquad
  \tilde{\sF} = \{A\in \sF\;|\;\mu_1(A)=\mu_2(A)\}
  \subseteq \sF
\end{align*}
It's easy to see that $\tilde{\sF}$ is a Dynkin system that contains
$\sE$.
So invoke Monotone Class Theorem~\ref{thm:mct-2} to get
$\tilde{\sF}=\sF$, i.e. $\mu_1(A)=\mu_2(A)$ for all $A\in\sF$.
\end{proof}
...
Well, this is obviusly weaker,\footnote{%
  ...
  compltely fail to work with only finite additivity.
}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Right continuous\footnote{%
      ...
      sense for distributions with mass points---we can interpret the
      jump size as the probability of point $x$ occuring.
    }
...
\begin{enumerate}
  \item If $y \geq x$, clearly $(-\infty,x]\subseteq (-\infty,y]$, so by
    monoticity of the underlying measure $P$:
    \begin{align*}
      P\big[(-\infty,x]\big]
      &\leq
      P\big[(-\infty,y]\big]
...
The next statement is very important as it says that writing down $F$
will \emph{uniquely} determine a probability measure on measurable space
$(\R,\sB(\R))$.
...
\begin{prop}
\emph{(From CDF to $P$ on $(\R^n,\sB(\R)^{\otimes n})$)}
...
\begin{defn}(Prob. Distribution and CDF of RV $X$)
Consider measurable function $X:(\Omega,\sF,P)\ra (\R,\sB(\R))$.
If $P$ is a probability measure on the underlying measurable space
$(\Omega,\sF)$, then the push forward measure $P \circ X^{-1}$ or $P_X$
is \emph{also} a probability measure---but on space
$(\R,\sB(\R))$---called the \emph{probability distribution of RV $X$}
satisfying
\begin{align*}
...
\end{align*}
the \emph{cumulative distribution function of RV $X$}.
\end{defn}
...
The above definitions are motivated as follows.  Suppose I want to
compute something as simple as $\E[g(X)]$ for some RV $X:\Omega\ra\R$
and function $g$. Here's one way to go about that
\begin{enumerate}[label=(\roman*)]
  \item Write down $\Omega$, the set of elementary events that could
    happen.
  \item Then specify measurable sets included in $\sF$--- the events or
    subsets of $\Omega$ whose probabilities I want to compute.
  \item Then specify a measure $P$ assigning probabilities to the
    measurable sets in $\sF$.
  \item Compute integral $\E[g(X)]=\int_\Omega g\circ X \; dP$
\end{enumerate}
Theoretically possible, sure.
But one big problem: I don't really know how to integrate over $\Omega$.
On the other hand, I might know how to integrate over $\R$, the range of
$X$. In fact, that's partly why I defined $X$ in the first place---so
that I wouldn't have to work exclusively with the more abstract measure
space $(\Omega,\sF,P)$. Rather than integrate over subsets of $\Omega$
(the domain of $X$ and $g\circ X$), I can change variables to integrate
over subsets of $\R$ (the range of $X$), using the preimage of $X$ to
invoke the underlying probability space $(\Omega,\sF,P)$ only
\emph{implicitly}:
\begin{align}
...
As a result, we actually work in a different way
to compute $\E[g(X)]$:
\begin{enumerate}[label=(\roman*)]
  \item Define a cumulative distribution function $F:\R\ra\R$. As
    discussed above, this is just a special type of function satisfying
    certain properties.
  \item Recognize that $F$ implicitly represents some distribution $P_X$
    that implies all of that other formal structure that makes
    probability work---a measurable space $(\Omega,\sF)$ and a random
    variable $X:\Omega\ra\R$ with probablity distribution $P_X$ on
    $(\R,\sB(\R))$ corresponding to a measure $P$ on underlying
    $(\Omega,\sF)$.
    The beauty is that you just have to ``recognize'' this step is
    happening. You don't actually have to do anything.
  \item Then by (ii), we have that
    \begin{align*}
      \E[g(X)] =
      \int_\R g(x) \; F(dx)
      =
...
specialize to a Riemann-Stieltjes integral) using function $F$ from $\R$
to $\R$.  That's huge. And as shown above, that is equivalent to
computing integral $\int_\Omega g \circ X \; dP$ on some underlying
probability space by Equation~\ref{omega-Px}.

Finally, we could have alternatively specified a density function
in step (i) rather than a CDF, in which case we have
\begin{align*}
  \int_\R g(x)f(x) \; dx
  =
  \int_\R g(x) \; F(dx)
...
\end{align*}
Note that the integrals on the first line all use the Lebesgue measure
(which might specialize to a Riemann or Riemann-Stieltjes integral).
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Definition of a measure $\mu$ and properties:
      complete

    \item
      Defn of Borel $\sigma$-algebra $\sB(\R)=\sigma(\sI)$
      gen from the collection of all half-open intervals
      \begin{align*}
        \sI := \left\{(a,b] \cap \R \; | \; -\infty\leq a < b \leq \infty\right\}
      \end{align*}
      ...
      \begin{enumerate}
        \item Corollary to MCTII: Suppose $\sE$ nonempty \& closed under
          finite intersections.
          If $P$ and $Q$ agree on $\sE$, then they agree on $\sigma(\sE)$.

          To prove this, define
          \begin{align*}
            \sD = \{A\in\sigma(\sE)|P[A]=Q[A]\}
          \end{align*}
          We have the following results
          \begin{enumerate}
            \item $\sD$ is a Dynkin system.
              Property 1: Assume $\Omega\in\sigma(\sE)$ and $P$ and $Q$
              agree there so good.
              Property 2: Also good.
              Property 3: Also good.
            \item $\sD$ contains $\sE$.  Also clear.
            %\item $\sD$ closed under finite intersections.
              %Consider $C=B\cap A$ where $P[A]=Q[A]$ and $P[B]=Q[B]$. Then
              %$P[A^c] = Q[A^c]$ and $P[B^c]=Q[B^c]$.
              %Hence
              %\begin{align*}
                %P[A] = P[C] + P[A\setminus C]
                %= P[C] + P[A\cap C^c]
              %\end{align*}
          \end{enumerate}
          By fact that $\sE$ nonempty and closed under finite
          intersections, (e) says $\delta(\sE)=\sigma(\sE)$.
          By (i) and (ii), $\sD\supseteq \delta(\sE)=\sigma(\sE)$.
          Hence $\sD\supset \sigma(\sE)$.
          $P[A]=Q[A]$ for all $A\in\sigma(\sE)$.

        %\item Lemma:
          %Given collection $\sE$ and measures $P,Q$ defined on
          %$\sigma(\sE)$, define collection
          %\begin{align*}
            %\sD=\{A\in\sigma(\sE)|P[A]=Q[A]\}
          %\end{align*}
          %Results:
          ...
          %\begin{align*}
            ...
            %\implies
            %\begin{cases}
              %A\in \sigma(\sE), \;P[A]=Q[A] \\
              %B\in \sigma(\sE), \;P[B]=Q[B] \\
            %\end{cases}
          %\end{align*}
          %is a Dynkin system that contains $\sE$, i.e.
          %$\sD\supset\delta(\sE)\supset\sE$.
          %Hence $\sD\supset\delta(\sE)=\sigma(\sE)$.
        %\item
          %If $\sP$ is closed under finite intersections,
          %$\sL$ is a Dynkin system, and $\sP\subseteq\sL$,
          %then we have that $\sigma(\sP)\subseteq\sL$.

          %In borel case, define $\sP$ to be $(-\infty,a]$ intervals.
          %Then definitely closed under finite intersections
          %and $\sB(\R)=\sigma(\sP)$.
          %Then define $\sL=\{A\in \sB(\R)|P[A]=Q[A]\}$.
          %$\sP\subseteq\sL$ by assumption.
          %Just need to show $\sL$ is a Dynkin system.

          %Typical: WTS some property holds for all sets in $\sF$.
          %Know collection of all sets with that property is a dynkin
          %system. Then find a collection of sets $\sP$ such that the
          %property holds, and show it's closed under finite intersections
          %and generates $\sF$.
        %\item
          %$\sI$ is nonempty and closed under finite intersections.
          %By (d), $\delta(\sI)=\sigma(\sI)$.

    \end{enumerate}
    We generally show some property holds on algebra $\sA$
    to claim that it holds on $\sigma(\sA)$, i.e. use (a)-(c).
    Or, can show some property holds for $\sE$ that is closed under
    finite intersections, then agree on $\sigma(\sE)$.

    Now, clearly set of all intervals like $(-\infty,x]$ is closed under
    finite intersections. Hence, if prob measures agree on those sets,
    then they agree on all sets in $\sigma$ algebra generated by that.
    Since those intervals generate $\sB(\R)$, we have uniqueness of
    measure.

    \item
      For non-empty index set $I$, have
      $\R^I=\{(\omega_i)_{i\in I}|\omega_i\in\R\}$
      i.e. the set of all functions $f:I\ra\R$.
      Let $q_i(f)=f(i)$ be the projection of the $i$th coordinate.
      Define product $\sigma$-algebra
      $\sB(\R)^{\otimes I}$ as smallest $\sigma$-algebra on $\R^I$ such
      that every $q_i$ measurable.
      Generated by all sets of the form $\{\omega\in\R^I|\omega_i\leq a\}$
      for $i\in I$ and $a\in\R$.
