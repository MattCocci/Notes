...
$\bsg_n(\bstildetheta)$, and then forming the sample analog $\bsg_N$ of
...
\begin{align}
  \bshata \; \bsg_N\big(\bshattheta(\bshata)\big) &= \bso
  \qquad\text{where}\quad
...
\end{align*}
where $\bshatW\pto\bsW$ (a positive definite matrix)
has asymptotic distribution
\begin{align*}
  \sqrt{N}(\bshattheta(\bshatW)-\bstheta)
  \quad&\dto\quad
  ...
  \bsG &:= \E\left[
    \frac{\partial \bsg(\bstheta)}{\partial \bstildetheta'}
  \right]
\end{align*}
To be clear, $\bsG$ is the Jacobian matrix of the function
...
matrix $\bshatW$.
We also define the Jacobian matrix of $\bsg_N$, evaluated at arbitrary
$\bsbartheta$:
\begin{align*}
  \bsG_N(\bsbartheta)
  =
...
\begin{align*}
  \bso &=
  \frac{\partial \bsg_N(\bshattheta)'}{\partial \bstildetheta}
  \, \bshatW \, \bsg_N(\bshattheta)
  =
  \bsG_N(\bshattheta)'
  \, \bshatW \, \bsg_N(\bshattheta)
\end{align*}
...
of each equation about the true value $\bstheta$:
\begin{align}
  \bso &=
  \bsG_N(\bshattheta)'
  \, \bshatW \,
...
also \emph{efficient} GMM estimators, i.e.\ estimators
$\bshattheta$ with the lowest possible asymptotic variance.
But this typically presents the following chicken and egg problem:
\begin{enumerate}[label=(\roman*)]
