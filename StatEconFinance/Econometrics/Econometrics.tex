...
\clearpage
\subsection{%
  Dirac, Discrete, (Abs.) Continuous Measures \& Density
}
\label{sec:abscts}
...
\begin{defn}
(Discrete, Continuous, Absolutely Continuous Meausures on $\R^d$)
...
\begin{itemize}
  \item \emph{Discrete}: If we can express $\mu$ as a sum of Dirac
    ...
    \end{align*}
    for $x_1,x_2,\ldots\in\Omega=\R^d$ and
    $p_1,p_2,\ldots\geq 0$
  ...
  \item \emph{Absolutely Continuous}: If $\exists$ a
    Borel-function\footnote{%
      Recall that $f$ is a Borel function if
      $f^{-1}(A) \in \sB(\R)^{\otimes d}$ for any open $A\subseteq\R$.
    }
    $f:\R^d\ra\R_+$ called a \emph{density} s.t.
    \begin{align*}
      \mu\big[
        (a_1,b_1] \times\cdots\times (a_d,b_d]
      \big]
    ...
    for all $A\in\sB(\R)^{\otimes d}$
    and Borel-measurable functions $g$
    \begin{align*}
      \mu[A] &= \int_A
      f(x_1,\ldots,x_d)\;
      dx_1\;\cdots \;dx_d
    ...
    it rules out weird, intractable cases.
\end{itemize}
\end{defn}
...
$\lVert X\rVert_p\leq \lVert X\rVert_q$ for
