...
\newcommand{\bshatx}{\boldsymbol{\hat{x}}}
\newcommand{\bshatX}{\boldsymbol{\hat{X}}}
\newcommand{\bshatV}{\boldsymbol{\hat{V}}}
...
\newcommand{\bshatS}{\boldsymbol{\hat{S}}}
...
\newcommand{\bsW}{\boldsymbol{W}}
\newcommand{\bshatW}{\boldsymbol{\hat{W}}}
  ...
  \item
    ...
    \begin{align*}
      \sqrt{N}\big(
        \bsa(\bshatbeta_{OLS})
        -
        \underbrace{\bsa(\bsbeta)}_{\text{$=0$ for $H_0$}}
      \big)
...
parameters $\bsbeta \in\Rk$ must equal the constant zero vector (without
loss of generality):
\begin{align*}
...
Therefore, we call $\bshatbeta$, the solution to the following system of
equations,
\begin{align*}
  \bsg_N(\bshatbeta) &= \bso
  \qquad\text{where}\quad
...
\end{align*}
the \emph{method of moments} estimator.
\\
\\
...
$\bshatW\pto \bsW$ as $N\ra\infty$.\footnote{%
  Note that $\bsW$ and $\bsw$ having nothing to do with each other
  directly. Just notation.}
...
conditions asymmetrically, emphasizing some over others.
We index $\bshatbeta$ by $\bshatW$ because the estimate depends
crucially on the choice of weighting matrix.
Most estimators that we care about can be expressed as a GMM estimator
given an appropriate choice of moment conditions and weighting matrix.
\end{defn}
\begin{rmk}
...
\clearpage
\subsection{Regression with Endogenous Regressors and Instruments}
...
\end{align*}
We construct the sample analog
\begin{align*}
  \bsg_N(\bsb) &=
