...
\clearpage
\subsubsection{$L^p$ Banach Space and $L^2$ Hilbert Space}
...
$|g|=\lambda |f|^{\frac{p}{q}}$.
\end{thm}
\begin{rmk}(Cauchy-Schwarz on $L^2$)
Theorem~\ref{thm:holder} is a strong result that holds for \emph{any}
arbitrary measurable functions. If we restrict ourselves to functions in
$L^2$ (which includes only those functions with finite $L^2$ norm),
then we have Cauchy-Schwarz on the inner product space $L^2$:
\begin{align}
  ...
  \label{L2cauchy}
\end{align}
This looks slightly weaker since
$\left| \int_\Omega f\cdot g\;d\mu \right|\leq
\int_\Omega \left| f\cdot g\right|\;d\mu$.
But notice our definition of $L^2$ and the $L^2$ norm implies that
$f,g\in L^2$ if and only if $|f|,|g|\in L^2$.
Using $|f|$ and $|g|$ instead of just $f$ and $g$ in
Inequality~\ref{L2cauchy} gets us our inequality of
Theorem~\ref{thm:holder}.

However, all told, it is still the case that Theorem~\ref{thm:holder}
properly generalizes Cauchy-Schwarz---both to cases where $p,q\neq 2$
and to arbitrary measurable functions $f,g\not\in L^2$.
\end{rmk}
...
show that given a probability measure $P$ on $(\R,\sB(\R))$, you can
...
\emph{implies} a unique probability measure on $(\R,\sB(\R))$ by the
Extension Theorems of Section~\ref{sec:extension}.
...
\begin{enumerate}
  \item If $y \geq x$, clearly $(-\infty,x]\subseteq (-\infty,y]$, so by
    monotonicity of the underlying measure $P$:
    \begin{align*}
      P\big[(-\infty,x]\big]
      &\leq
      P\big[(-\infty,y]\big]
...
The next two propositions are very important.
The first says that if you want to check whether two measures $P$ and
$Q$ on $(\R,\sB(\R))$ are equal, just check their CDFs. You don't have
to bother checking equality for \emph{every} Borel set in $\sB(\R)$.
Just compare measures on Borel sets of the form $(-\infty,a]$, which is
entirely equivalent to the very simple task of comparing whether CDFs
$F_P$ and $F_Q$ are equal. That's a truly massive simplification.
...
$F:\R\ra\R$ satisfying a few simple properties) \emph{implies} a unique
probability measure on measurable space $(\R,\sB(\R))$ and all the deep
measure-theoretic structure that comes along with that implication.
That's also huge.

These two propositions are really the culmination of all the confusing,
abstract, annoying Monotone Class and Extension Theorem set theory
nonsense.
...
\begin{align}
  ...
  \label{pqagree}
\end{align}
Then then they are identical for all measurable sets in $\sB(\R)$:
\begin{align*}
  P[A] = Q[A]
...
\end{prop}
\begin{proof}
By Expression~\ref{pqagree}, measures $P$ and $Q$ agree on all sets in
the collection $\sI_{(-\infty,a]}$ of Definition~\ref{defn:Iinftya},
which generates $\sB(\R)=\sigma(\sI_{(-\infty,a]})$ according to
Proposition~\ref{prop:borelalt}.  Moreover, by
Proposition~\ref{prop:borellambda}, this generating collection
$\sI_{(-\infty,a]}$ is nonempty and closed under finite intersections.
Hence, we can apply Theorem~\ref{thm:PQunq} (a corollary to a Monotone
Class Theorem) to conclude that measures $P$ and $Q$ agree on the
...
measure $P_F$ on measurable space $(\R,\sB(\R))$ s.t.
\begin{align}
...
\end{prop}
\begin{proof}
Expression~\ref{PFdefine} defines a probability measure $P_F$ on the
...
Hence, Caratheodory's Theorem~\ref{thm:cara} says that $P_F$ can be
extended to the entire generated $\sigma$-algebra,
$\sB(\R)=\sigma(\sI_{(a,b]})$.
The measure $P_F$ is also clearly $\sigma$-finite, hence this extension
is also unique by the same Caratheodory's Theorem~\ref{thm:cara}.
\end{proof}
...
\begin{enumerate}
  ...
  \item $\lim_{x^k_i\ra-\infty} F(x^k) = 0$ holds for each $i$,
    regardless of whether the other coordinates are converging to
    $-\infty$.
\end{enumerate}
\end{defn}
...
because, if $n=1$, it reduces reduces to simply $F(b) - F(a) \geq 0$.
For $n=2$, it reads
\begin{align*}
  0 \leq
  \Delta_{a_1,b_1}
...
unique probability measure $P_F$ on measurable space
$(\R^n,\sB(\R)^{\otimes n})$ such that
\begin{align*}
  P_F\big[\, (a_1,b_1]\times\cdots\times(a_n,b_n] \,\big] =
  \Delta_{a_1,b_2}
  \cdots
  \Delta_{a_n,b_n}
  F(x_1,\ldots,x_n)
