...
``distribution of $f$.'' See Section~\ref{sec:expdist}.
\end{rmk}
...
abstract measurable space $(\Omega,\sF)$. That's surely an improvement,
but we still don't necessarily know how to integrate with respect to
$P_X$ in practice. Therefore, we would like to replace
...
\subsection{Univariate Normal/Gaussian Distribution}
...
\begin{align*}
  \int_\R e^{xz} e^{-\frac{1}{2}z^2}\; dz
  =
  \int_\R e^{-\frac{1}{2}\left(z^2-2xz\right)}\; dz
  =
  \int_\R e^{-\frac{1}{2}\left(z^2-2xz+x^2-x^2\right)}\; dz
  =
  e^{\frac{x^2}{2}}
  \int_\R e^{-\frac{1}{2}\left(z-x\right)^2}\; dz
\end{align*}
But the final integral in the chain is simply the integral of the
density of a $N(x,1)$ RV (without the multiplying constant).
...
Plug in $x=iu$ (which we can do by analytic continuity and complex
...
We define the $d$-dimensional \emph{multivariate normal}
or \emph{Gaussian} RV as that random vector $X\sim \calN(\mu,\Sigma)$
with characteristic function
\begin{align*}
  \varphi_X(u)
  = e^{iu'\mu-\frac{1}{2}u'\Sigma u}
\end{align*}
given some $\mu\in\R^d$ and symmetric positive-semidefinite matrix
$\Sigma\in\R^{d\times d}$. Moreover, we call $X$
\begin{itemize}
  \item \emph{Regular Normal} if $\Sigma$ \emph{invertible}, hence
    positive definite
  \item \emph{Degenerate Normal} if $\Sigma$ \emph{not invertible},
    hence only still positive semidefinite matrix.
    In this case random vector $X$ lives on a proper subspace of $\R^d$.
\end{itemize}
\end{defn}
...
definition of the distribution given above in Equation, but
