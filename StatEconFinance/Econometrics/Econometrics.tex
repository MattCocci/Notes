...
\clearpage
\subsection{Extremum Estimators and M-Estimators}

Extremum estimators are quite important. Virtually all estimators of
interest---MLE, NLS, GMM---are (or can be written as) extremum
estimators. We also introduce a special type of extremum estimator
called the M-estimator, which also nests as special cases MLE and NLS.


\begin{defn}(Extremum Estimator)
We call $\bshattheta$ an \emph{extremum estimator} if it solves
\begin{align*}
  \bshattheta = \argmax_{\bstildetheta\in\bsTheta}\;
  Q_N(\bstildetheta)
\end{align*}
for some sample objective function $Q_N(\bstildetheta)$ that depends on
sample $\{\bsy_n\}\nN$ and converges to corresponding population
objective function $Q(\bstildetheta)$ as $N\ra\infty$ where
\begin{align*}
...
\end{align*}
for some function $q(\bsy_n,\bstildetheta)$. Implicit in this setup is
that the true $\bstheta$ we hope to estimate satisfies
$\E\big[q'(\bsy_n,\bstheta)\big]$ where $q'$ is the gradient of $q$
...
\begin{align}
  ...
  \notag
\end{align}
Since $\E[q'(\bsy_n,\bstheta)]=0$, we can compute limiting variance in
Expression~\ref{mestvar} by computing just the second moments.
\end{prop}

\begin{proof}
Recall the maximization problem that defines our M-estimator:
\begin{align*}
  \bshattheta =
  \argmax_{\bstildetheta\in\bsTheta}\;
  \frac{1}{N}
...
$P$-first order conditions (ommitting unnecessary $1/N$):
\begin{align*}
  \bso &=
  \sumnN q'(\bsy_n,\bshattheta)
\end{align*}
Since $q'$ is the derivative with respect to vector $\bstildetheta$
(i.e.\ the gradient) evaluated at $\bshattheta$, this is $P$-equations
in $P$-unknowns. We can do a Taylor Expansion of the $p$th equation
about the (unknown) true value $\bstheta$:
\begin{align*}
  0 &=
  \sumnN q'_p(\bsy_n,\bshattheta)
  =
  \sumnN
  \big[
  q'_p(\bsy_n,\bstheta)
  +
  q''_p(\bsy_n,\bstildetheta_p)
  (\bshattheta-\bstheta)
  \big]
\end{align*}
where $\bstildetheta_p$ lies in between true $\bstheta$ and estimated
$\bshattheta$.
There is one for each of the $P$ equations. Rearranging,
\begin{align*}
  ...
  \bigg]
\end{align*}
By the LLN and the CMT, the first converges in probability to
\end{proof}
