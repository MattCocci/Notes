...
\clearpage
\section{Classical Linear Regression: The Exact Regression Model in
Finite Samples}
...
multivariate normal distribution. So we have a multivariate normal
wrapped around a quadratic form.
\end{proof}
...
\begin{enumerate}
  ...
  \item \emph{Ergodic Stationarity}:
    ...
    ergodic. Much weaker than iid.
  \item \emph{Predetermined Regressors}:
    $\E[\bsx_n (y_n - \bsx'_n \bsbeta)]=\E[\bsx_n \varepsilon_n]= \bso$
    for all $n$.

    This is weaker than requiring $\E[\varepsilon_n|\bsx_n]=0$.
    It is also weaker than strict exogeneity, which requires
    $\E[x_{ni}\varepsilon_m]=0$ for all $n,m$.
    Here, we rule out only contemporaneous ($n=m$) correlation between
    regressor $\bsx_n$ and error $\varepsilon_n$.
  ...
  \item \emph{Martingale Difference Sequence $\bsx_n\varepsilon_n$}:
    ...
    i.e.\ $\E[\bsx_n\varepsilon_n^2\bsx_n]<\infty$, and is is a
...
\end{defn}
\begin{rmk}
This model permits conditional heteroskedasticity. Since $(y_n,\bsx_n)$
is stationary, the error term $\varepsilon_n$ is also stationary so that
$\E[\varepsilon^2_n]$ is constant across $n$. But the setup above
allows $\E[\varepsilon_n^2|\bsx_n]$ to depend upon $\bsx_n$.
\end{rmk}
...
\begin{enumerate}
  \item \emph{Consistent}: Under Assumptions 1-4 above, the OLS
    estimator is consistent.
  \item \emph{Asymptotic Normality}: Under assumptions 1-5
    \begin{align*}
      \sqrt{N}(\bshatbeta_N- \bsbeta) \dto
      N(0,\E[\bsx_i\bsx_i']^{-1}\;\E[\bsx_n\varepsilon^2_n\bsx_n']\;\E[\bsx_i\bsx_i']^{-1})
    \end{align*}
...
\begin{enumerate}
  ...
  \item
    ...
    $N(0,\Var(\bsx_n\bsvarepsilon_n))$ by the CLT.
    By Slutsky, the desired result follows.
