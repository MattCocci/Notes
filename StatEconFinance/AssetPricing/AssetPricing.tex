...
Agent problem: two periods, period endowments $e_t$ and $e_{t+1}$, rate
of \emph{time} (but \emph{not} risk) discounting $\beta$. Can buy/sell
any amount of asset (hence can make arbitrarily small marginal
adjustments), cost today $p_t$, payoff $x_{t+1}$ (random var realized at
$t+1$).
\begin{align*}
  \max_{\xi\in\R}
  \;
...
\begin{align}
  ...
  %\label{mdf}
\end{align}
Eq.~\ref{pemx1} holds \emph{after} agent has made all
desired investments \& reached optimal allocation.\footnote{%
...
\begin{align}
  ...
  \label{ret}
\end{align}
Eq.~\ref{pemx1} is a map/operator $p[x]$ from RV payoff $x(\omega)$ to
...
\begin{align}
  ...
  \label{rf}
\end{align}
To interpret pricing Eq.~\ref{pemx1}, use Eq.~\ref{rf} and
...
Assume complete markets with contingent claims (or a market where claims
can be replicated/synthsized).
Define the contingent claim price vector in payoff space $\R^S$:
\begin{align*}
  \bsp\bsc
...
Euclidean space. \emph{But} every dot/inner product is really $\E[xy]$
underneath.
...
Assume $\underline{X}\subsetneq \R^S$, i.e. tradeable assets do not
\emph{span} the state space. As an example consider the picture below
where $S=3$, but $\underline{X}=\R^2$, i.e. an infinite plane extending
out in each direction from the edges of the triangle.
...
Suppose further that we don't have an economic model giving us a
discount factor $m$ that prices payoffs $x\in\underline{X}$ according to
$p=\E[mx]$. Can we still rescue something like the pricing formula
$p=\E[mx]$?

\begin{assump}
For all $\forall x_1,x_2\in \underline{X}$ and $a,b\in\R$,
we might assume
\begin{itemize}
  ...
  \item[A3.] \emph{Absence of Arbitrage}:
    If $x(\omega)\geq x'(\omega)$ for all $\omega\in\Omega$ and
    strict for at least one $\omega$, then $p[x]>p[x']$.

    Given A1 and A2, equivalent to:
    If $x(\omega)\geq 0$ for all $\omega$ and
    strict for at least one, then $p[x]>0$.
    In words, ``No zero-cost portfolios that might pay.''
\end{itemize}
Note that people colloquially ``no arbitrage'' to refer to violations of
LOOP, \emph{not} A3.
\end{assump}
...
\begin{itemize}
  \item
    \emph{($\Rightarrow$)}
    Given a discount factor $m$ from an economic model pricing all
    assets $p=\E[mx]$, A2 (LOOP) holds.
  \item
    \emph{($\Leftarrow$)}
...
\clearpage
\subsubsection{Mean-Variance Frontier}

Rewrite the covariance
\begin{align*}
  \E(R^i) - R^f
  &= -\frac{1}{\E(m)} \; \rho_{m,R^i} \; \sigma_{R^i} \; \sigma_m
\end{align*}
Since correlation $|\rho_{m,R^i}|\leq 1$,
\begin{align*}
  \frac{\left\lvert\E(R^i) - R^f\right\rvert}{\sigma_{R^i}}
  &\leq \frac{\sigma_m}{\E(m)}
\end{align*}
$R^f$ risk-free so $\sigma_{R^f}=0$ and $\sigma_{R^i-R^f}=\sigma_{R^i}$,
hence can write the above as a bound on the exp. Sharpe Ratio for excess
return $R^e=R^i-R^f$:
\begin{align*}
  ...
  &\leq \frac{\sigma_m}{\E(m)}
\end{align*}
Whether we think of returns $R^i$ or excess return $R^e=R^i-R^f$,
returns of all assets lie within a wedge-shaped region.
Boundary of the region is the \emph{mean-variance frontier}.
