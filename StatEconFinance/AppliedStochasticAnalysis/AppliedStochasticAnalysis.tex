...
give you know more information about its future path.
\begin{rmk}
...
\begin{enumerate}
  \item Define $u_t^{(x)} := \mathbb{E}[f(X_t)|X_0=x]$. Then given
    boundary condition $u^{(x)}_0=f(x)$,
    \begin{align*}
  ...
  \item For a Markov process solving a time-inhomogeneous SDE
    ...
    \begin{align*}
      \frac{\partial p}{\partial s} + \mathscr{L}_x p = 0
      \qquad s<t
    \end{align*}
    where $\mathscr{L}_x$ denotes that the derivatives in $\mathscr{L}$
