...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    $C_Y(t)=2C_X(t)$ as our covariance function for $Y$.

  \item % Question 7b
    Since $X_t$ is Gaussian, $A_t=X_t$ is Gaussian. Since $A_t$ and
    $B_t$ have identical distribution, $B_t$ is Gaussian. As the sum of
    two Gaussian RV's, $Y_t$ is also Gaussian.

    Since $Y_t$ is Gaussian, its spectral process $Z(\lambda)$ is Gaussian,
    implying that the $\xi_k$ are Gaussian. They are independent because
    $Z(\lambda)$ has orthogonal increments; therefore, the $\xi_k$ are
    uncorrelated.
